...
| **1 · Ядро** (`shared-core`)                         | ✅ Готово                        | Модули (ниже; счётчик и состав двух сборок — [`state.md`](state.md) §9, единственный дом этих чисел), микроядро + `advanceTo`, детерминизм (sfc32 + golden), fail-secure, **squadron** (свободный полёт эскадрилий/ракет, `squadron.strike`/`return`/`arrived`), **pointDefense** (реактивное ПВО: залп по эскадрильям в радиусе, перезарядка 20 мин)                                                                                                                                                                                                                                                              | Канон собирает весь каталог ядра (36/36, CORE-PARITY 2026-08-26); у прототипа нет `pve`/`station`/`visibility` — см. `state.md` §9 |
