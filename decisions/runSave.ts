 ...
 *
 * Та же форма едет и в ОБЛАКО (AUD-24, поле `state` облачного конверта, `cloudSync.ts`).
 * Раньше облако везло только компактный дескриптор ({@link ./portableRun}), и другое
 * устройство пересобирало мир с карты главы — а новый мир выгоднее проигрываемого: вход
 * со второго устройства стирал поражение. Размер это позволяет: лимит `setData` — 200 КБ
 * (`YAG-0.1`/`YAG-0.2`; «10 КБ» относилось к `setStats`), а снимок главы — до 36 КБ.
 * Доводы за дескриптор — частота записи и сеть на телефоне — держит частота: мир уходит в
 * облако раз в 30 с и на уходе со страницы, а не на каждое событие. Третий довод —
 * совместимость версий — остаётся за дескриптором: он и есть запасной путь, когда снимок
 * после обновления игры миром не становится.
 */
