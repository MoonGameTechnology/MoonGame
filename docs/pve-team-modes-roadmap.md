...
**Чего нет:** 1v1/3v3/4v4 в `NetworkMatchMode` (Фаза 1), AI-оркестратора PvE
(Фаза 5) — без него волны спавнятся, но не наступают. Фазы 0, 2, 3 и 4 закрыты.
...
PVE-0.2 modeId в MatchConfig       ✅ резолв в конструкторе MatchRoom, E_UNKNOWN_MODE
   ...
   │
   ├─ PVE-5.* AI-оркестратор       ← СЛЕДУЮЩИЕ (PVE-3.3 и PVE-4.3 закрыты)
...
**Порядок сборки (остаток):** PVE-5.* (AI-оркестратор — без него волны стоят на
месте) → PVE-1.* (командные форматы) → PVE-6.* (доки). Фазы 0, 2, 3 и 4 закрыты.
