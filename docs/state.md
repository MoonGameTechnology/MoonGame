...
Итого **917 ключей**. Статичный узел в разметке ПУСТ, ключ стоит в ЗНАЧЕНИИ атрибута
...
локалью. Оставшиеся **425** вызовов (`main.ts` 415, `sandbox.ts` 10) ещё держит **мост**
`/localization/legacy/en.ts` (старый msgid = русская строка, 616 записей); мост только
