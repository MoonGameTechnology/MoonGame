 ...
 * версия склеивала ровно `fleet.retreat`+`fleet.move`, и две другие зависимые
 * последовательности планировщика молча разъезжались по разным порциям:
 *   · `fleet.split` → `fleet.move` того же флота (`ai.ts:723`);
 *   · `shuttle.loadTroops` → `shuttle.strike` той же эскадры (`ai.ts:1188`), про который
 *     в `ai.ts` прямо написано «груз кладут в трюм ОТДЕЛЬНЫМ приказом, и он идёт ПЕРЕД
 *     ударом: два приказа одного тика применяются по порядку».
 * Между порциями проходит ≥16 мс с отпущенным `driversBusy`, за которые успевают и
 * стоячие драйверы, и `advanceTo`. Разъехавшаяся пара — это десант, который взлетел
 * пустым, и флот, отделённый и никуда не ушедший: редьюсер отклонит второй приказ, и
 * бот молча не сделает задуманное. Общий актор ловит все три случая и любой будущий,
 * не требуя правок здесь.
 *
