...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5768 зелёных** (62 skip, 433 файла; с `DATABASE_URL` — 5830 без пропусков: все пропуски — тесты durable-пути, которым нужна база).
