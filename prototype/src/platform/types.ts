...
export interface PlatformSave {
  ...
  /** Примет ли площадка этот снимок по размеру. Нет метода — лимита нет (локальное
   *  хранилище). Хост спрашивает ДО записи (AUD-24): не влезший мир забега уходит из
   *  конверта, а профиль всё равно доезжает. */
