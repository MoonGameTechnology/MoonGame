...
export const BuildingDefSchema = z.object({
   ...
   *  СТРОКОЙ в редьюсере (`planet.buildings.some(...)`, причём в двух местах) — в
   *  данных о нём не было ни слова, и прочитать «правила игры» было негде. Дефолт `1`
   *  сохраняет прежнее поведение для всего каталога; здание, которому можно стоять в
   *  нескольких экземплярах, объявляет это числом, а не правкой ядра. */
