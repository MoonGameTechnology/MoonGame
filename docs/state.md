...
**Прогон идёт НЕДЕЛЮ и без досрочной победы по очкам (AI-BAL-6, заказ владельца).** Раньше
матч кончался порогом очков на 4-м дне из 60 — до того как война успевала что-то решить, и
измерение было бинарным. Теперь у всех матчей одна длина (7 игровых дней, исход `timeout`),
а в конце ранжирование по очкам: боёв стало вдвое больше (5400 → 10800), неисследованных
техов 9 → 5 из 25. Рейтинг по очкам сразу вскрыл то, чего бинарная метрика не показывала:
старт `C5R1` даёт 1494 очка против 889 (+68%), а фракции при win-rate 50/50 расходятся на
25% (crimson 1323 против azure 1060). Живого игрока это не касается: недельный конфиг живёт
только в харнесах, правила победы игроку задают `protoKernel.ts` и `netserver.ts`, и границу
стережёт `aiProfile.test.ts`. Отдельно: разные семьи сидов дают
