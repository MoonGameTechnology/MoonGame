...
if [ -n "$GIT_TOKEN" ]; then
  ...
  GIT_TOKEN="$GIT_TOKEN" GIT_ASKPASS="$askpass" GIT_TERMINAL_PROMPT=0 \
    git pull "https://x-access-token@github.com/MoonGameTechnology/MoonGame.git" "$branch" \
  ...
  # git@github.com:MoonGameTechnology/MoonGame.git) so the box holds no reusable secret at all.
