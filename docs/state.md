...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5170 зелёных** (58 skip, 392 файлов).
...
том, что наземных боёв стало МЕНЬШЕ (9043 → 8200): занятие пустого мира с орбиты попадает в
ту же корзину, что и высадка, — `planet.captured` знает только `via: 'arrival'` и «прочее»
(AI-BAL-10).
...
−20%/−18% — значит отступление флот сохраняет, а выигрыш съедает что-то из добавленного рядом
(осада держит флот под `orbital_aa`, раскол плодит цели). Открыто как AI-BAL-11.
...
меняет хозяина в основном прилётом (**86%** после BAL-4, было 88%), snowball 63–69%, кредиты декоративны, дерево технологий — очередь по
цене; исходов `elimination` на равнозначных стартах не осталось вовсе (матчи доживают до
