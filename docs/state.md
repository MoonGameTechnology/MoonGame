...
- **UI — тактический пульт (DEFCON-вайб):** векторно-каркасный стиль на чёрном.
  ...
  - **Опциональная флагманская консоль на ПК и планшетах:** переключатель в настройках
    ...
    **Единое окно флота:** исходные `#side` и `#cmdbar` — соседние секции одного
    `#holo-selection-window`. Состав, состояние и настоящие кнопки приказов
    находятся в общей рамке; повторная командная шапка скрыта. При прицеливании
    информация скрывается по прежним правилам, команды отмены/подтверждения
    остаются в том же окне. Положение не зависит от флота, орбиты или камеры.
    ...
    **Нестабильная кромка:** `drawGlassRim` рисует вдоль четырёх краёв мира
    короткие холодные нити с мягким мерцанием и небольшим «кипением». Геометрия
    и толщина масштабируются с рамкой карты. Девять групповых проходов Canvas
    обходятся без покадрового blur; выключенное свечение оставляет три слабых
    линейных прохода. Используется тот же `hologramTime`, поэтому пауза и
    выключенное движение замораживают кромку вместе с остальной проекцией.
