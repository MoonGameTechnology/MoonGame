...
function commanderStoreContract(
  ...
  describe(`CommanderStore — ${name}`, () => {
    ...
    //
    // TEST-4. Очки берутся ОТ ЧАСОВ, а не константой. Раньше здесь стояли 1_000_001…3,
    // одни и те же в каждом прогоне: строки копились в общей таблице, и после полутора
    // десятков прогонов подряд собственные три аккаунта переставали помещаться в окно
    // `topXp(50)` — тест падал на живой базе разработчика и выглядел как регрессия в
    // чужом диффе (ловил дважды за одну сессию, оба раза лечил `truncate`).
    // Монотонная база делает этот прогон заведомо верхушкой доски, сколько бы прогонов
    // ни накопилось до него. `xp` — bigint, миллисекунды в него влезают с запасом.
    it('topXp returns the board in descending order, capped by the limit', async () => {
      ...
      await store.creditMatch(uniq('bm'), [
        { accountId: lo, xp: base + 1 },
        { accountId: hi, xp: base + 3 },
        { accountId: mid, xp: base + 2 },
      ]);
