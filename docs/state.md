...
Итого **642 ключа**. Статичный узел в разметке ПУСТ, ключ стоит в ЗНАЧЕНИИ атрибута
...
локалью. Оставшиеся **743** вызова (`main.ts` 733, `sandbox.ts` 10) ещё держит **мост**
`/localization/legacy/en.ts` (старый msgid = русская строка, 842 записи); мост только
...
и без старой формы, таблицы копии отдают ключи, мост не растёт). Последний тест держит
пять таблиц (GLOSSARY, INTROS, FIRST_GOALS и оба тура), где ключ уходит в `t()`
переменной и потому невидим разбору литералов.
