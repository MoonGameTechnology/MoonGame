...
describe('aiOrders — исследование технологий (AI-BAL-1)', () => {
  ...
  it('БОТ НЕ ЗАКАЗЫВАЕТ ГРАНТ-УЗЕЛ — ни при какой казне (BAL-15)', () => {
    // Сторож против регресса, который убил исследование целиком. Грант-узлы (`grantOnly`:
    // мета-прокачка командира и усиления забега) бесплатны и мгновенны, поэтому в
    // сортировке бота «дешёвое и быстрое вперёд» они встают ПЕРВЫМИ. Пока запрет жил
    // только в обработчике, бот каждый шаг просил такой узел, получал `E_GRANT_ONLY` и
    // до настоящего дерева не доходил НИКОГДА — «техи: 0 исследовано» в каждом прогоне.
    //
    // Проверяется свойство, а не список имён: заказ бота никогда не грант-узел. Список
    // устарел бы молча, свойство — нет.
    const s = game2();
    const rich: GameState = {
      ...s,
      players: { ...s.players, p2: { ...s.players.p2!, resources: { credits: 9e6, metal: 9e6 } } },
    };
    const broke: GameState = {
      ...s,
      players: { ...s.players, p2: { ...s.players.p2!, resources: {} } },
    };
    for (const state of [s, rich, broke]) {
      for (const order of research(aiOrders(state, 'p2', 'expand', 'strong'))) {
        const id = techOf(order);
        expect(data.technologies[id]?.grantOnly ?? false, `бот заказал грант-узел «${id}»`).toBe(
          false,
        );
      }
    }
  ...
  it('пустая казна → бот МОЛЧИТ: бесплатных узлов в дереве не осталось (BAL-15)', () => {
    // Прежде здесь стояло обратное утверждение — «берёт только бесплатное (6 мета-техов)»,
    // и оно было верным ровно до того, как грант-узлы заперли. Сегодня бесплатное в
    // каталоге — это РОВНО грант-узлы, брать их нельзя, а всё настоящее дерево платное.
    // Значит честный ответ на пустую казну — отсутствие заказа, а не заказ даром.
    const s = game2();
    const broke: GameState = {
      ...s,
      players: { ...s.players, p2: { ...s.players.p2!, resources: {} } },
    };
    expect(research(aiOrders(broke, 'p2', 'expand', 'strong'))).toHaveLength(0);
  });
