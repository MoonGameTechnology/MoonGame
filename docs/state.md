...
Итого **778 ключей**. Статичный узел в разметке ПУСТ, ключ стоит в ЗНАЧЕНИИ атрибута
...
локалью. Оставшиеся **576** вызовов (`main.ts` 566, `sandbox.ts` 10) ещё держит **мост**
`/localization/legacy/en.ts` (старый msgid = русская строка, 733 записи); мост только
