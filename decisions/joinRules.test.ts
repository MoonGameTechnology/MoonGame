...
describe('вход в матч — строка запроса', () => {
  ...
  // ADDR-2/ENTRY-4: совет учёных выбирался на экране входа и клался в ССЫЛКУ
  // (`seatJoin.ts`), а сервер читал его из запроса (`matchApi.ts`) — но клиент между ними
  // его не переносил, и состав до партии не доезжал вовсе.
  it('несёт совет учёных — иначе выбор со экрана входа теряется по дороге', () => {
    expect(joinQuery('p2', 'azure', ['overseer', 'polymath'])).toBe(
      '?slot=p2&faction=azure&sci=overseer%2Cpolymath',
    );
  });

  it('пустой совет параметра не даёт — «не выбирал» и «выбрал пусто» разные утверждения', () => {
    expect(joinQuery('p2', undefined, [])).toBe('?slot=p2');
