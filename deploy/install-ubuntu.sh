...
# `age` — шифрование бэкапов (SE-3.4). Ставим здесь, потому что `backup.sh` без
# настроенного получателя ОТКАЗЫВАЕТСЯ работать, и обнаруживать отсутствие пакета в
# момент первого ночного крона — худший вариант. На уже развёрнутой машине пакет
# доставляется руками (`apt install age`): установщик до неё не доезжает — ровно та
# ловушка, что описана в env-keys.sh.
if ! command -v age &> /dev/null; then
    log_info "Установка age (шифрование бэкапов)..."
    apt-get install -y age && log_success "age установлен" \
        || log_error "age не установился — бэкапы придётся настроить вручную (см. deploy/README.md)"
fi
