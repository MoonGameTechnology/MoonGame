...
- **UI — тактический пульт (DEFCON-вайб):** векторно-каркасный стиль на чёрном.
  ...
  - **Опциональная флагманская консоль на ПК и планшетах:** переключатель в настройках
    ...
    **Космическая местность** вместо парящих бейджей типа получает узор внутри того
    же полигона: объёмные туманности, разряды ионных бурь, дуги вспышек, астероиды и
    обломки. Мягкие градиенты туманностей/вспышек запекаются в статический слой;
    живой слой сохраняет ограниченное число линий и фрагментов. Камни отличаются
    от сломанных рёбер обломков. Неизвестная местность остаётся скрыта; используется прежняя память
    разведки. Планеты, крепости, здания и реальные сигналы сохраняют свои обозначения.
    Перелив стекла — холодная бирюзово-сине-фиолетовая лента с мягкими краями,
    тонкими каустиками и скользящим жемчужным бликом; золотистых оттенков нет.
    Она обрезается скруглённой рамой мира, рисуется под игровыми объектами и
    использует градиенты без покадрового blur. Выключенное свечение убирает
    широкое сияние, сохраняя слабую кромку. Волна и материалы используют `hologramTime`, поэтому
    замирают вместе с существующей декоративной анимацией. Движение регулируется
    только в настройках (`void.motion`); игровую обработку радарных контактов
    оно не отключает. Статические материалы/рама кэшируются, живые — отсечены экраном.
