...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 3788 зелёных** (58 skip, 297 файлов).
...
**0 схлопнутых**; без ключа было 38, сейчас 24 (технологии закрыл отдельный проход) — эти
не схлопываются и чинятся дозаведением, разбивка и причины ниже.
...
непокрытой сущности, и на записи, для которой ключ уже появился. Записей 24 —
`buildings` 11, `factions` 6, `scientists` 5, `planetTypes` 1, `sectorKinds` 1 (14
технологий по дороге закрыл отдельный PR). Снять 17 из них нельзя, пока не решено AUD-8:
у прототипа для тех же сущностей другие имена и частью другие id.
