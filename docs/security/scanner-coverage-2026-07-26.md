...
- `bearer.yml`: конфиг для Bearer SAST (исключает `prototype/`), но **нет джобы**
  в `security.yml` — мёртвый конфиг (см. G-7). _Файл удалён 2026-08-04 — поправка
  в G-7._
...
> **Поправка 2026-08-04 (KICS).** Пробел A05 закрыт: в `security.yml` добавлена джоба
> `kics` — единственный из доступных движков, у которого Docker Compose есть как
> платформа (у Checkov его нет). Пока информационная, до триажа базовой линии.
> Две особенности, из-за которых наивное включение не сработало бы:
>
> - KICS опознаёт compose ТОЛЬКО по расширению `.yaml` (его апстрим-док platforms.md), а
>   у нас `.yml` — джоба делает
>   копии внутри себя и возвращает пути в SARIF к настоящим файлам. Переименовывать
>   `deploy/docker-compose*.yml` нельзя: на имена завязан боевой путь деплоя.
> - Значение `--type` в апстрим-доке KICS указано как `docker-compose`, и CLI его
>   ОТВЕРГАЕТ: печатает справку и выходит с 126. Рабочее — `DockerCompose`, как
>   перечислено в выводе `kics scan --help`. Документация разошлась с бинарём, источник
>   истины — справка самого образа. Поймано первым же прогоном (PR #473), потому что
>   джоба падает при `scanned < copied`; без этого правила в отчёт уехало бы «0 находок».
> - Джоба ПАДАЕТ, если просканировано меньше файлов, чем скопировано. Без этого пустой
>   отчёт был бы неотличим от чистого — тот самый fail-open, ради которого заведены
>   сентинелы `status-<key>.json`.
>
> Повод: `POSTGRES_PASSWORD:-void` в прод-пути прожил незамеченным именно потому, что
> compose не смотрел никто.
...
> **Поправка 2026-08-04 (G-7 закрыт удалением).** Развилка решена в пользу удаления,
> и вот почему — отчёт называл конфиг «мёртвым», не назвав причину смерти. Причина
> нашлась в истории: Bearer **реально работал**, но на GitLab (`.gitlab-ci.yml:108`
> — это зафиксировано в `2026-06-26-docs-audit.md`). При переезде на GitHub Actions
> джобу не перенесли, а конфиг остался — то есть это не «забыли включить», а хвост
> завершённой миграции.
>
> Против «добавить джобу» три довода, каждый проверен, а не предположен:
> 1. **Находок нет.** Аудит `audit-2026-06-27.md` (§ про `innerHTML`) зафиксировал
>    единственный класс находок Bearer на этой кодовой базе — CWE-79 на `innerHTML`
>    в `prototype/`, уже экранированных через `escapeHtml`. Ровно этот путь конфиг
>    и исключал. На боевой поверхности (`shared-core`, `server`, `action-layer`,
>    `client`) Bearer не находил ничего.
> 2. **SAST-движка уже два** — Semgrep (паттерны, блокирующий) и CodeQL (поток
>    данных). Уникальный угол Bearer — поток ЧУВСТВИТЕЛЬНЫХ данных (PII-классификация
>    до стока). Для игры без персональных данных сверх идентификатора сессии это
>    добавляет мало.
> 3. **Цена не нулевая.** Джоба — это ещё один сторонний образ с примонтированным
>    репозиторием, тот самый класс, ради которого в пайплайн только что добавлены
>    дайджест-пины и harden-runner. Платить им за движок из п.1–2 не стоит.
>
> Инструмент при этом живой (v2.0.1, февраль 2026, Cycode) — если появятся реальные
> персональные данные (почта в дайджестах, платежи), пункт стоит открыть заново, уже
> отдельным кирпичом с разбором цены. Удаление обратимо одним `git revert`.
