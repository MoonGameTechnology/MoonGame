...
### HTTPS-2.1 · Caddy/Nginx-пример с ACME перед сервером `[ops][sec]` ✅ (код; домен — deploy-time вход) — M → SE-1.2

**Цель:** на VPS публичный `443/wss`, сервер — на `127.0.0.1`.
**Сделано (Caddy-путь):**

- `deploy/Caddyfile` — Caddy авто-Let's Encrypt: `reverse_proxy server:8788`, прозрачный
  `wss`, `http→https`, HTTP/3, `X-Forwarded-Proto`/`-Host`/`-For` из коробки (минимальный конфиг);
- `deploy/docker-compose.tls.yml` — оверлей с сервисом `caddy` (`:80`/`:443` + HTTP/3, том
  `caddy-data` для персиста сертификатов), `TRUST_PROXY=1` серверу (доверять proxy-заголовкам за краем);
- база: `${SERVER_BIND:-0.0.0.0}` (не ломает LAN-путь; TLS-деплой ставит `SERVER_BIND=127.0.0.1`
  → плоский порт только loopback); `serve.sh` — `HOST` конфигурируем (деф. `0.0.0.0`);
- запуск: `DOMAIN=… SERVER_BIND=127.0.0.1 docker compose -f docker-compose.yml -f docker-compose.tls.yml up -d`;
  health/доки — `deploy/README.md`. Домен/DNS — deploy-time вход оператора (в код не убирается).
  **Готово:** `https://<домен>/` и `wss://<домен>/matches/…` через Caddy; прямой plain-порт наружу не торчит.

### HTTPS-2.2 · Жизненный цикл сертификата `[ops][sec]` ✅ (авто-продление + staging via Caddy) / ⏳ (мониторинг истечения) — S → SE-1.2

**Сделано (в HTTPS-2.1):** Caddy продлевает сертификат сам (без cron/таймера), сертификаты
персистятся в томе `caddy-data`; staging-эндпоинт ACME и `tls internal` для отладки задокументированы
в `deploy/Caddyfile` (анти-rate-limit Let's Encrypt). Предусловие — домен и DNS (A/AAAA на VPS).
**Остаток (⏳):** мониторинг даты истечения (алерт за N дней) — увязать с SE-8.2 (алертинг).
