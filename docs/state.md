...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 4877 зелёных** (58 skip, 372 файла).
...
- **Ступени коридора — прогрессия ОДНОЙ способности**, а не три кнопки в ростере.
  ...
  `state/corridor.ts`).
- **Ускорение коридора — привилегия БЛОКА, а не всех, кто в него вошёл** (решение
  владельца). Хук `fleet.speed` даёт ×(1+`speedBonus`) флотам владельца лейна И его
  союзников (`isAllied` — capability `diplomacy`, без модуля честное чтение D1-стойки,
  вторая копия правила не заводится, MAPSHARE-1). Противнику общий коридор даёт только
  ДОРОГУ: он проходит, но на своей скорости. Правило действует на всех ступенях, поэтому
  на коридоре поверх УЖЕ существующей лейны (`addedLink: false`) союзник тоже поедет
  быстрее — там коридор никому не закрывает проход, он только ускоряет.
