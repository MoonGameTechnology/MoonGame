 ...
 * 2. **Порт без предела** (SHU-5.1, резолюция владельца 2026-09-26; прежде здесь был
 *    предел `E_HANGAR_FULL`). `shuttleBay` порта — только ворота: стоит порт — он держит
 *    сколько угодно машин.
...
describe('ангар космопорта — постройка челнока (правила 1–3)', () => {
  ...
  it('ПОРТ БЕЗ ПРЕДЕЛА: заказ сверх `shuttleBay` проходит и весь встаёт в ангар', () => {
    // `shuttleBay` порта в фикстуре — 2; десять машин всё равно встают.
    const s = built(world(), 'interceptor', 10);
    expect(hangarOf(s)).toEqual([{ unit: 'interceptor', count: 10 }]);
    // И уже стоящие места не занимают: следующий заказ тоже проходит.
    expect(code(s, build('interceptor', 5))).toBeNull();
    // `time.advanced` не срезает «лишних» — лишних у порта не бывает.
    expect(hangarOf(advance(s, s.time + 3_600_000))).toEqual([{ unit: 'interceptor', count: 10 }]);
  });
...
describe('ангар космопорта — потеря порта (правило 4)', () => {
  ...
  it('ОДИН ИЗ ПОРТОВ СНЕСЁН — пока стоит второй, гибнуть некому', () => {
    // Порт без предела (SHU-5.1): вместимость — ворота, и пока стоит хоть один порт,
    // ангар цел целиком.
    let s = built(
      world([
        ['spaceport', 30],
    ...
    expect(hangarOf(after)[0]?.count).toBe(3);
  });
