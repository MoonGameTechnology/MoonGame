...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5849 зелёных** (62 skip, 436 файлов; с `DATABASE_URL` — 5911 без пропусков: все пропуски — тесты durable-пути, которым нужна база).
...
- `diplomacy?: Record<pairKey, DiplomaticStance>` — попарные дип-отношения (`war`/`peace`/
  ...
  **И обратное (CMB-7): бой идёт, только пока стороны ВРАЖДЕБНЫ.** Раньше вражду
  ...
  гарнизон под перемирие не попадает — стойки у него нет. **ПВО бьёт залпами, двумя ярусами** (не непрерывно):
