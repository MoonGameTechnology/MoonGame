...
| **1 · Ядро** (`shared-core`)                         | ✅ Готово                        | Модули (ниже; счётчик и состав двух сборок — [`state.md`](state.md) §9, единственный дом этих чисел), микроядро + `advanceTo`, детерминизм (sfc32 + golden), fail-secure, **shuttle** (свободный полёт эскадрилий/ракет, `shuttle.strike`/`return`/`arrived`), **pointDefense** (реактивное ПВО: залп по эскадрильям в радиусе, перезарядка 20 мин)                                                                                                                                                                                                                                                              | Канон собирает весь каталог ядра (36/36, CORE-PARITY 2026-08-26); у прототипа нет `pve`/`station`/`visibility` — см. `state.md` §9 |
...
## Ядро — модули (все написаны и покрыты тестами; СОБИРАЮТСЯ не все — `state.md` §9)
...
>
> **Причина третьего ❌ с тех пор сменилась (2026-09-09).** Механизмы auth, TLS и гейта
> построены И принуждаются: `PROD=1` не даёт серверу стартовать без
> `AUTH_JWT_SECRET`+`GATE=1`+TLS+`SEAT_LOCK=1`+`ALLOWED_ORIGINS`
> (`checkProductionReadiness`). Публичный запуск держат уже другие пункты списка ниже:
> постура конкретного развёртывания (1), контент на длинный матч (4), OIDC (5) и
> мультипроцессный масштаб (6).
...
3. **TLS на self-hosted пути ЕСТЬ** (HTTPS-2.1/2.2, RS-5.1) — двумя независимыми путями:
   `deploy/Caddyfile` + `docker-compose.tls.yml` (Caddy сам берёт и продлевает
   сертификат Let's Encrypt, сервер уходит на loopback) либо нативный TLS в самом
   сервере по `TLS_KEY_FILE`+`TLS_CERT_FILE`. Cloudflared-туннель больше не
   единственный выход наружу. Осталась приёмка HTTPS-7.1 на живом домене (нет
   plaintext, HSTS, testssl-A) — её без домена и DNS не провести.
