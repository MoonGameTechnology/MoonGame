...
поэтому он собирается своим `deploy/caddy/Dockerfile` — тот же исходник v2.11.4, но свежим
тулчейном (go1.26.8 из пиненного `caddy:2-builder-alpine`) плюс единственный модульный бамп
`x/text 0.39.0`. Это закрыло 14 находок в его бинаре из 22, включая ВСЕ СЕМЬ достижимых у
нас; приёмку сторожит джоба `trivy-caddy` — она сверяет по паре (id, пакет), что целевые
находки исчезли, и красная, если пересборка перестала их закрывать. Пакеты рантайм-базы
(curl, openssl) не трогались: они недостижимы, и `apk upgrade` — отдельное решение. Свой
...
у сервера, и это разобрано, а не срезано: в бинаре остаются находки С фиксом (CRITICAL в
`x/crypto`, три HIGH в grpc), закрываемые только бампом модулей поверх `go.mod` caddy — от
чего SEC-31 отказался ради непоявления непроверенных апстримом комбинаций в терминаторе
TLS, и все они уже разобраны по символам как недостижимые (SEC-32). Вместо этого блокируют
ПРИЁМКА (пересборка всё ещё закрывает свои 14 находок — тот же скрипт, что у `trivy-caddy`,
одна копия списка на репозиторий) и СМОУК (образ стартует, HEALTHCHECK доходит до healthy,
`:80` отвечает). Подпись обоих образов проверяет один `deploy/verify-image.sh`: ограничение
