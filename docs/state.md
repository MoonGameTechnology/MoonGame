...
- `data/` — контент в JSON, вкл. карты `data/maps/*` (skirmish-1, ava-duel-1 2×1,
  ...
  **Песочница (dev-only, `prototype/src/sandbox.ts`):** галочка «🧪 Песочница» в
  ...
  бессмертный дом · заморозка очередей у всех) и разовые команды (мгновенная
  перезарядка скиллов командиров · +2000 любого ресурса · прекратить все войны →
  ...
  работает (сервер авторитетен). Тесты — `sandbox.test.ts` (6).
