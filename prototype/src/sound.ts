...
export type SoundId = 'tap' | 'close' | 'error' | 'send' | 'radar' | 'start';
...
export const SOUND_MIN_GAP: Record<SoundId, number> = {
  ...
  error: 0.25,
  send: 0.4,
  // Развёртка красит контакты пачками: за один кадр рука может пересечь несколько
  // целей, а полный оборот занимает ~10 с. Зазор держит из этого редкую капель, а не
  // трель — и он ЕДИНСТВЕННОЕ, что стоит между «радар работает» и «радар бесит».
  radar: 0.5,
  start: 1.0,
};
...
export const SOUND_PATCHES: Record<SoundId, SoundPatch> = {
  ...
  // Засечка развёртки: гидролокаторный пинг — чистый тон с еле слышным сползанием
  // вниз и тень октавой ниже. Самый громкий эхо-посыл в каталоге и при этом самый
  // тихий пик: сам щелчок почти не слышен, слышен ХВОСТ, уходящий в пустоту. Это
  // единственный звук, который игрок не вызывал сам — поэтому он должен читаться как
  // фон приборной панели, а не как уведомление.
  radar: {
    echo: 0.5,
    voices: [
      { wave: 'sine', f0: 1244, f1: 1174, gain: 0.07, attack: 0.004, decay: 0.16, lp: 3200 },
      { wave: 'sine', f0: 622, gain: 0.03, attack: 0.006, decay: 0.2, lp: 1400 },
    ],
  },
