...
**✅ PvE и командные режимы (PVE-0..5) — механика собрана, играть пока негде.**
...
SEC-25 2026-08-26), а её неустранимые CVE лежат в `.trivyignore` — каждая запись с
пакетом, причиной недостижимости и датой ре-ревью, и с обязательным пере-ревью ВСЕХ
записей при бампе дайджеста. **Гейт `trivy image` фильтрует по ЧИНИМОСТИ, а не по
