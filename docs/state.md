...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5519 зелёных** (58 skip, 419 файлов).
...
- `combat.tick` — почасовые раунды: атакующая сторона бьёт `attack`, стоящий
  ...
  **ВАЖНО: хук покрывает ТОЛЬКО этот канал.** Каналов урона четыре, и остальные три зовут
  `applyDamageToSide` напрямую, с уроном, посчитанным мимо хука: `orbital` (ПВО и
  бомбардировка), `artillery` (огонь с дистанции), `squadron` (удар эскадрилий). Значит там
  не работает вся точка расширения — пассив фракции, бонус урона от технологий, способности
  и ауры героя, модификаторы форта и типа планеты. Собственный комментарий модуля при этом
  называет хук «admiral / tactic / **bombardment** extension point», то есть бомбардировку
  покрывать предполагал. Замысел это или регрессия разбора монолита — открыто как
  `CORE-DMG-1` в `backlog.md`; цена выросла с AI-BAL-7, когда осада стала массовой.
