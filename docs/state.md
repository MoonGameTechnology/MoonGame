...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5829 зелёных** (62 skip, 436 файлов; с `DATABASE_URL` — 5891 без пропусков: все пропуски — тесты durable-пути, которым нужна база).
...
- `diplomacy?: Record<pairKey, DiplomaticStance>` — попарные дип-отношения (`war`/`peace`/
  ...
  **Объявление войны само сводит стоящих рядом (CMB-5):** `combatModule` слушает
  ...
  мирно, после объявления войны не дрались, пока кто-нибудь не сдвинется. **ПВО бьёт залпами, двумя ярусами** (не непрерывно):
