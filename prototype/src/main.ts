...
function updateThreatAlerts(): void {
  ...
  for (const a of threatAlerts(sightings, s.time, threatMemory)) {
    note(
      a.left === null
        ? t('threat.here', { node: placeName(a.node) })
        : t('threat.incoming', { node: placeName(a.node), dur: stewFmtDur(a.left) }),
      a.node,
    );
  }
}
...
function drawFogMarker(c: { x: number; y: number }, id: string, mem: Snapshot | undefined, lod: MapLod): void {
  ...
  if (mem) {
    ...
    cx.fillText(placeName(id), c.x + 13, c.y - 1);
  ...
  const text =
    news.key === 'log.hero.died'
      ...
      : news.key === 'log.hero.died.bare'
        ? t(news.key, { who })
        : t(news.key, { who, at: news.at !== undefined ? placeName(news.at) : '' });
  note(text, news.at);
}
...
function tellBuild(kind: BuildLogKind, p: Record<string, unknown>): void {
  ...
  const where = placeName(at);
  const text = line.needsLevel
    ? t(line.key, { b, lvl: String(p.level), at: where })
    : t(line.key, { b, at: where });
...
function render(now: number) {
  ...
  if (lod.art < 1) {
    ...
    for (const n of MAP) {
      ...
      if (p.owner === ME && known(n.id) && n.sector === 'planet') {
        ...
        cx.fillText(placeName(n.id), c.x + lod.markerRadius + 6, c.y - 1);
      }
    }
    cx.restore();
  }
  ...
  for (const n of MAP) {
    ...
    if (n.sector === 'asteroid' && !holographicMapOn()) {
      ...
      cx.fillText(placeName(n.id), c.x + 16, c.y - 1);
    ...
    // sensor-range ring (dashed, faint) — only where a radar actually stands (PVR-6.19):
    // drawn on every world it read as a radar the world doesn't have. Same gate as the
    // range finder (`radarShown`, rule 11), so it can't leak a hidden radar through fog.
    if (radarShown({ detailed: seesDetails(p), reach: planetRadar(p) })) {
      cx.save();
      cx.setLineDash([3, 5]);
    ...
    cx.fillText(placeName(n.id), c.x + R + 12, c.y - 1);
    if (line.do === 'stats') {
  ...
  const grantLabel = (g: IntelGrant): string =>
    g.kind === 'treasury'
      ...
      : g.kind === 'fleets'
        ...
        : t('log.spy.what.world', { at: placeName(g.target) });
  const rows = myIntel()
