...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5525 зелёных** (58 skip, 420 файлов).
...
- **Командный бой (AVA-0, первый шаг к AvA без мета-слоя):** тумблер «⚔ Командный бой» в
  ...
  **«Мои партии» — главный экран, а не вкладка обозревателя (ADDR-4):** раздел в `#hp-home`
  ...
  Строка несёт состояние (день, места, идёт/завершён), полный адрес и кнопки «Войти» /
  «Скопировать адрес». Адрес — `shareAddress` (правило 7 `matchAddress.ts`): строится на
