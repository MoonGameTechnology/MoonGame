...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5812 зелёных** (70 skip, 434 файла; с `DATABASE_URL` — 5882 без пропусков). Пропуски — тесты durable-пути и учение бэкапов, которым нужна база; в CI база есть, но нет пакета `age`, поэтому там 5881 и пропущен один тест — реальный криптокруг (проводку шифрования CI проверяет заглушкой, см. §10).
