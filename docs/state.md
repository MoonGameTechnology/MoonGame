...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 4962 зелёных** (58 skip, 377 файлов).
...
- **Командный бой (AVA-0, первый шаг к AvA без мета-слоя):** тумблер «⚔ Командный бой» в
  ...
  **Сетевой вход = соло-экран «Настройка схватки» в сетевом режиме (ENTRY-2/3/4):**
  ...
  открыт. Выбор уходит в адрес входа (`?join&slot&faction&sci`), сервер разбирает и
  ...
  **Адрес партии ≠ приглашение (ADDR-2):** захватные параметры живут в строке ровно до
  входа — `decisions/matchAddress.ts` (14 тестов, общее решение обоих клиентов) вычищает
  `slot`/`faction`/`sci` из `location` после успешного захвата (`claimDone`, тот же приём
  и та же причина, что у `?reset=<token>`), чужие параметры не трогая; сидящему в партии
  ссылка адресована как адрес, поэтому захватных параметров он и не шлёт. Иначе отданная
