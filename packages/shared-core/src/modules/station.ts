...
/** Ядро крепости — здание, которое приходит с конверсией. Экспортируется по той же причине,
 *  что {@link STATION_COST}: кнопка клиента спрашивает про него ворота технологий
 *  (`technologiesUnlocking`), и своя строка `'starfort'` у неё — второй дом правила. */
export const STATION_CORE = CORE_BUILDING;
