...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5680 зелёных** (61 skip, 429 файлов).
...
- **Челноки** (shuttles-roadmap; переименовано из «эскадрилий» в SHU-0.1 — трейт
  ...
  **МОДЕЛЬ ДОСТРАИВАЕТСЯ:** удар «прилетел → ударил → сразу домой» — SHU-1.2; описанное
  ниже про вылет крылом — ТЕКУЩЕЕ поведение до него.
  `interceptor` +
  `strike_carrier` строятся; носитель отделяет крыло в отдельный быстрый флот через
  `fleet.split` (кнопка «Запустить челноки»); дерётся обычным боем, `orbital_aa`
