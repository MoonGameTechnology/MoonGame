...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5023 зелёных** (58 skip, 381 файл).
...
## 6. Данные (`data/*.json`, версия `0.1.8`)
...
- **modes** (`modes.json`, `GameModeDefSchema`, PVE-0.1): режим = именованный пресет
  ...
  Два пресета. `standard` — те же числа, что и базовые правила `victoryModule`
