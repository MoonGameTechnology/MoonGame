...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5759 зелёных** (70 skip, 435 файлов; с `DATABASE_URL` — 5829 без пропусков). Пропуски — тесты durable-пути и учение бэкапов, которым нужна база; в CI база есть, но нет пакета `age`, поэтому там 5828 и пропущен один тест — реальный криптокруг (проводку шифрования CI проверяет заглушкой, см. §10).
...
- **Челноки** (shuttles-roadmap; переименовано из «эскадрилий» в SHU-0.1 — трейт
  ...
  **ИНТЕРФЕЙС (SHU-3.1, половина):** вкладка «Эскадра» панели мира показывает АНГАР ПОРТА
  ...
  «На борт»/«В порт» перегружают между портом и стоящим у мира носителем. До этого
