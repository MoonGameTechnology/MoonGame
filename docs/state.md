...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5773 зелёных** (62 skip, 434 файла; с `DATABASE_URL` — 5835 без пропусков: все пропуски — тесты durable-пути, которым нужна база).
...
> **`prototype/src/prototypeData.ts` (1319 строк) больше НЕ каталог игры** — он оставлен
> эталоном сравнения, пока сторож паритета доказывает, что переезд принёс ровно известную
> дельту. Снос — шаг C кирпича CONV-12.
> **Дрейф был остановлен (CONV-11) и повёрнут вспять (CONV-12a).** Сторож
> `prototype/src/contentParity.ts` + `contentParity.test.ts` (13 тестов) сверяет каталоги
> пер-полю против базового списка `prototype/content-parity-baseline.txt` — **98 расхождений**
> (274 → 261 CONV-15 → 210 CONV-17 → 216 с новым контентом HC-3 → 213 CONV-16 → 99 CONV-12a
> → 98 попутной правкой блока ROS).
> Красное в ОБЕ стороны: новая строка = дрейф вырос (чинят, а не дописывают в список),
> пропавшая = долг закрыт и запись убирают тем же PR (конвенция `.trivyignore`).
...
> определению.
> **Правило выбора стороны — не вкус, а вопрос «кто читает».** Для каждого поля: читает ли
> его модуль из графа, который крутит ПРОТОТИП? Читает — побеждает прототип, потому что его
> числа мерили девять кирпичей BAL. Не читает никто или читает только каноническая
> машинерия (`pve`, `factionStart`, `visibility`) — остаётся канон: такая строка ничего не
> стоит и закроется сама, когда умрёт второй каталог.
