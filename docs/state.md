...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 4847 зелёных** (58 skip, 370 файлов).
...
`techTree.ts` вместо `scientistModule`. **CONV-1…4 закрыты** — сведены форс-марш,
мгновенный ремонт, доковый ремонт (`econScrews` → `fleetRepairModule`), гражданский налог
и столица: копии из `prototype/src` удалены, оба хоста на модулях ядра. Тесты прототипа
прошли по ядру без правок — версии совпадали, и это и было проверкой. Попутно закрыта
дыра с `__proto__` в идентификаторе флота (`ownFleet` вместо сырого индекса) сразу в трёх
обработчиках прототипа, а в ядро перенесены случаи, которые проверялись только у
прототипа. Осталось: `sessionMarket`, `fleetLaunch`, `standingOrders`, хелперы
`squadron.ts` и врезка про совет учёных в `techTree.ts`.
