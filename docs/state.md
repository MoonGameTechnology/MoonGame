...
decisions/       ОБЩИЕ решения обоих клиентов (`decisions/README.md`): чистая функция + типы + тесты, ни DOM, ни сети. Заведена, потому что вынесенные REFM-модули легли внутрь `prototype/src` и новому клиенту недоступны (после MIG-1 там остаётся 166 таких модулей, из них 151 чистый) — он пишет свои `welcomeScreen`/`matchHud`/`mapRender`/`camera`. Устроена как `/localization`: корневая папка, оба клиента тянут относительным путём, конфигурации сборки не нужно. Явные упоминания — `vitest.config.ts` (include тестов), `prototype/tsconfig.json` (include типов), `prototype/src/i18n.test.ts` (папка в списке потребителей локализации). Контракт папки машинный — `shared.test.ts` читает КАТАЛОГ и держит три правила: из `/decisions` не импортируют `prototype/` (иначе решение молча перестаёт быть общим), исходники чистые, у каждого решения есть тест рядом; все три проверены мутацией. Жильцов **22** (было 5): MIG-1 перевёз цепочку сетевого входа целиком — адрес сервера, вход, обзор матчей, обмен сессии на место, дозвон, приветствие, жизнь сокета, маршруты приказа и отказа. Умолчание прежнее — старое переезжает, когда его и так трогают; разом везут только связную цепочку, которой второму клиенту не хватает сейчас, и только при чистоте, тестах и НЕИЗМЕННОМ числе тестов в гейте (условия — `decisions/README.md`)
...
и край экрана). Токены дополнительной темы — `packages/client/src/theme.ts`.
...
- **UI — тактический пульт (DEFCON-вайб):** векторно-каркасный стиль на чёрном.
  - **Карта = голографическая проекция:** исходные координаты, размеры провинций и
    граф маршрутов сохранены. Тёмный звёздный фон (`spaceBackdrop.ts`, WebP 43 КБ)
  ...
  - **Опциональная флагманская консоль на ПК и планшетах:** тумблер «Голография»
    ...
    приказов не меняются ради оформления. Переключатель есть также в настройках.
    ...
    **Космическая местность** вместо парящих бейджей типа получает узор внутри того
    же полигона: волокна туманностей, разряды ионных бурь, дуги вспышек, астероиды и
    обломки. Неизвестная местность остаётся скрыта; используется прежняя память
    разведки. Планеты, крепости, здания и реальные сигналы сохраняют свои обозначения.
    Мягкая волна стекла и движение материалов используют `hologramTime`, поэтому
    замирают вместе с существующей декоративной анимацией. Тумблер «Анимация» внизу
    меняет тот же `void.motion`, что настройки; игровую обработку радарных контактов
    он не отключает. Статические материалы/рама кэшируются, живые — отсечены экраном.
