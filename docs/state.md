...
одним кирпичом старт `C5R1` 100% и snowball 100%, и AI-BAL-2 показал, почему: исход у
100% матчей — `score`, а `scoreValue` есть и у зданий, и у провинций, так что любая
постройка приближает победу. Снежный ком живёт в СЧЁТЕ, а не в бою: матч кончается на
4-м дне из 60, до того как война успевает что-то решить. Отдельно: разные семьи сидов дают
