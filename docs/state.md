...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5833 зелёных** (70 skip, 434 файла; с `DATABASE_URL` — 5903 без пропусков). Пропуски — тесты durable-пути и учение бэкапов, которым нужна база; в CI база есть, но нет пакета `age`, поэтому там 5902 и пропущен один тест — реальный криптокруг (проводку шифрования CI проверяет заглушкой, см. §10).
...
- `combat.tick` — **первый раунд на самой встрече** (CMB-4: `startBattle` назначает его
  ...
  **Линии получения урона** `front/mid/rear` (`util/combat.ts`, ROS-2.1 — линий ТРИ):
  ...
  армию раздача не режет. Своей линии у артиллерии больше НЕТ: трейт `artillery`
  означает, что по корпусу не проходит ответный залп, пока атакует его сторона
  (`sparesArtillery`, см. ниже), а сам корпус стоит в тылу, как заявлено данными. Пул HP стека с переносом, `unit.died`. **Это же — сейчас — и наземная фаза** (десант/гарнизон),
  ...
  **Через хук идут ВСЕ каналы урона** (CORE-DMG-1; решение владельца, ОБРАЩЁННОЕ — первая
  ...
  `shuttle`. Каждый канал зовёт хук со своей фазой — `orbital` / `bombard` / `standoff` /
  `pointDefense` / `intercept` / `shuttle` / `returnFire` (ответка по челнокам, ROS-2.2),
...
- **Линии и ответный огонь (ROS-2.1).** Залп делится по ТРЁМ линиям 50/30/20
  ...
  остаток идёт вперёд по строю. Раздача ответного залпа по АТАКУЮЩЕЙ стороне идёт с
  флагом `sparesArtillery`: корпуса с трейтом `artillery` из неё исключаются, и их доля
  перераспределяется тем же правилом «отсутствующей линии» — то есть артиллерия бьёт
  безнаказанно. Поблажка снимается, когда щадить больше некого: если кроме артиллерии на
  стороне никого не осталось, залп приходит по ней (иначе флот из одних пушек был бы
  неубиваем). Щадимые пересчитываются на КАЖДОМ проходе перелива — последний обычный
  корпус может погибнуть в этом же залпе, и остаток обязан найти цель. Прогноз боя
  (`previewBattle`) зовёт ту же `damageUnits` с тем же флагом, иначе разошёлся бы с боем.
...
- **Челноки** (shuttles-roadmap; переименовано из «эскадрилий» в SHU-0.1 — трейт
  ...
  **Живут В КОСМОПОРТЕ (SHU-1.1):** `planet.hangar` — не флот и не гарнизон; вместимость
  ...
  не поднимает.
  **ЭСКАДРА — СОЕДИНЕНИЕ (SHU-4.2, манифест 13→14):** ангар мира и трюм носителя это
  ...
  **ОТВЕТКА В МОМЕНТ УДАРА (ROS-2.2, заказ владельца п. 4):** цель огрызается по вылету
  ...
  этот залп добьёт, всё равно успевает выстрелить (та же одновременность, что у
  артиллерии). Волна, сбитая целиком, домой не летит и из `state.strikes` уходит.
...
- **Just-in-time интро механик (ONB-3)** — при **первом** контакте с продвинутой механикой —
  ...
  мид-боя в спокойный момент), первый `fleet.barrage` (обстрел — карточка ЖДЁТ носителя: дальнобойных корпусов в ростере нет с ROS-2.1, а кадр после ROS-2.1a такому флоту наводку и не предлагает, так что до ракет этот триггер не срабатывает); fail-secure
