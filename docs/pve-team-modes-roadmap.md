...
**Чего нет:** 1v1/3v3/4v4 в `NetworkMatchMode` (Фаза 1); ИНТЕГРАЦИИ оркестратора
(PVE-5.2) — сама тактика написана и покрыта тестами, но её пока никто не зовёт,
поэтому волны спавнятся и стоят. Фазы 0, 2, 3 и 4 закрыты.
...
### PVE-5.2 · Интеграция в `MatchRoom` `[srv]` ⏳ — M (не S)
**Цель:** кто-то на сервере регулярно зовёт `pveOrders` и подаёт результат в комнату.

**Шов НЕ так очевиден, как выглядел в плане — это выяснилось при PVE-5.1, и решать
его надо осознанно, а не «дописать вызов в tick».** Что мешает:
- **`MatchRoom.tick()` синхронный, а `submitServerAction` — асинхронный** (на durable
  комнате он идёт через actor-mailbox с commit-before-broadcast). Вызов
  fire-and-forget внутри `tick()` — это гонка с собственным коммитом комнаты, ровно
  тот класс бага, который уже ловили (bug-hunt CRIT: silent state loss).
- **`clockDriver` общий для ВСЕХ комнат** — PvE-специфика в нём это протечка режима
  в инфраструктуру.
- **Условие «это PvE» брать не из `config.modeId === 'pve_waves'`** (жёсткая привязка
  к id пресета), а из наличия `state.pve` — тем же правилом, что `victoryModule` и
  сам `pveModule`.
- **Счётчик `seq` кто-то должен ВЛАДЕТЬ** между тиками: повтор id — это приказы
  второй волны, съеденные кэшем квитанций как ретраи.
**Кандидаты на шов:** опциональный `serverOrders?: (state) => Action[]` в
`MatchRoomOptions` (комната сама решает, когда безопасно подать), либо хост-цикл
рядом с `runServerStanding` в `netserver.ts`, как сделано для `aiOrders` прототипа.
**Готово, когда:** NPC действует в живом PvE-матче, приказы проходят гейты, и тест
показывает движение волны без гонки с персистом.
...
PVE-0.2 modeId в MatchConfig       ✅ резолв в конструкторе MatchRoom, E_UNKNOWN_MODE
   ...
   ├─ PVE-5.2 интеграция в сервер  ← СЛЕДУЮЩИЙ (шов описан в кирпиче)
   │
...
**Порядок сборки (остаток):** PVE-5.2 (интеграция — без неё волны стоят на месте)
→ PVE-1.* (командные форматы) → PVE-6.* (доки). Фазы 0, 2, 3 и 4 закрыты.
...
□ AI-тактика — Фаза 5 БОЛЬШЕ НЕ опциональна по факту: волны спавнятся у логова
  Роя и стоят там, потому что двигать их некому. До оркестратора PvE-режим
  проходится сам собой (люди прилетают и режут статичные флоты), так что это
  следующий по ценности кирпич, а не «опционально».
