...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5584 зелёных** (61 skip, 428 файлов).
...
## 6. Данные (`data/*.json`, версия `0.1.11`)
...
> с базовым списком `prototype/content-parity-baseline.txt` — **210 расхождений** (замер
> 2026-08-27 дал 274; CONV-15 закрыл 13, CONV-17 — ещё 51, оба ниже). Красное в ОБЕ стороны: новая
