...
(+15% скорости флота) берёт **59.0%** побед (349/592, **+4.36σ**) при 1502 очках, `violet`
(+5%/+5%) — 44.4% (263/592, −2.71σ), `azure` 49.2%, `crimson` 47.5%. Держится во всех
четырёх семьях по отдельности. Ведущее объяснение (не измерено): партия решается темпом
раздела карты, а скорость флота конвертируется в него напрямую; `violet` же не размен, а
просто меньшее число в обеих осях. Открыто как **BAL-11** — какую цифру двигать, решает
владелец. Что осталось: территория по-прежнему
