...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5765 зелёных** (70 skip, 434 файла; с `DATABASE_URL` — 5835 без пропусков). Пропуски — тесты durable-пути и учение бэкапов, которым нужна база; в CI база есть, но нет пакета `age`, поэтому там 5834 и пропущен один тест — реальный криптокруг (проводку шифрования CI проверяет заглушкой, см. §10).
...
- **Челноки** (shuttles-roadmap; переименовано из «эскадрилий» в SHU-0.1 — трейт
  ...
  **НОСИТЕЛЬ — ВТОРАЯ БАЗА (SHU-2.1):** `shuttleBay` стал ещё и статом КОРПУСА, поэтому
  ...
  суммирует `aaDamage` и по гарнизону, и по зданиям. **Имя игроку — «Орбитальное ПКО»**
