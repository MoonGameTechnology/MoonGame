 ...
 * Port of the prototype's `recap.ts` (`prototype/src/recap.ts`) into `shared-core`
 * so both the client (on return) and the server (`packages/server/src/push.ts`,
 * a future push digest) can build off the SAME logic instead of forking it.
 *
 * Pure: no DOM, no i18n, no clock — the caller feeds a flat event stream (already-
 * localised text + a `since` game-time) and gets back a sorted digest. Importance is
 * read from the event's own emoji marker (present in every locale — see
 * `/localization`), so it needs no schema change to the note() call sites.
 */
