...
- **UI — тактический пульт (DEFCON-вайб):** векторно-каркасный стиль на чёрном.
  ...
  - **Телефонный тактический интерфейс — первый этап:** `mobileHud.ts` переносит
    ...
    одинаковых уведомлениях WebView; мерцание на физическом Android ещё требует
    проверки на устройстве. Отдельная адаптация стратегических экранов — следующий этап.
...
- Валидаторы: `src/smoke.ts` (Node-сценарий ядра) и `uitest.mjs` (headless-DOM
  ...
  не проверка физического Android WebView. `perf.mjs` явно открывает карту перед
  замером idle/pan/zoom, чтобы не измерять пустой путь за стартовым экраном.
