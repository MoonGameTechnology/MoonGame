...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 4853 зелёных** (58 skip, 370 файлов).
...
`techTree.ts` вместо `scientistModule`. **CONV-1 и CONV-2 закрыты:** `forcedMarch`,
`instantRepair` и доковый ремонт (`econScrews` → `fleetRepairModule`) сведены — копии
удалены, оба хоста на модулях ядра; все 17 тестов прототипа прошли по ядру без правок
(версии совпадали), а сама смена закрыла у прототипа дыру с `__proto__` в идентификаторе
флота (`ownFleet` вместо сырого индекса) сразу в трёх обработчиках.
