...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5161 зелёных** (58 skip, 392 файлов).
...
- **Карта — «колесо» из десяти секторов (`map.ts`, BAL-1 → BAL-9):** 121 провинция — ровно **30
  ...
  они по BAL-3 декоративны, — то есть **разнообразие сейчас косметическое** и станет настоящим,
  когда вторичные ресурсы начнут ограничивать. Сумма
...
**AI-BAL-9 — рынок**, и он оказался мёртв НЕ по той причине, что записана в кирпиче. Бот
научился звать `market.take` (пороги — его же собственные цены, новых не заведено), и сделок
стало по-прежнему 0. Прибор доработан до ответа: `econplaytest` считает теперь и выставленные
лоты — **48 лотов, 0 сделок**, потому что оба места просят металл и предлагают еду, энергию и
микроэлектронику, то есть спрос и предложение не встречаются никогда. Книга сошлась на
единственном ресурсе, который кому-то нужен: бот закрывает чужую ЗАЯВКУ на металл из избытка,
не проседая ниже собственной границы нужды. Оборот **0 → 2.3k кредитов**, заявок на металл
12 → 28. Пока металл — единственный ограничивающий ресурс (BAL-3), еда и энергия остаются
товаром без покупателя, сколько бы их ни выставляли.
...
работает как очередь по цене. **И два прежних вывода оказались артефактами бинарного
...
они кредитами, энергией и едой — то есть **косметически**, пока BAL-3 не даст этим ресурсам цену.
