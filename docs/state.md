...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 4958 зелёных** (58 skip, 377 файлов).
...
## 6. Данные (`data/*.json`, версия `0.1.7`)
...
них. Обратно: канонический гоняет `visibility`, `station` и `pve`, которых нет у
...
`'7'`). У `MODULES` прототипа такого сторожа нет — он не версионируется поматчево.
