...
- **Карта — «колесо» из десяти секторов (`map.ts`, BAL-1):** 121 провинция — ровно **30
  ...
  Раскладка: общая ступица + десять секторов по 36°, каждый сектор — один и тот же шаблон
  из 12 узлов (четыре кольца по три), повёрнутый на k·36°. Карта переходит сама в себя при
  повороте на 36°, поэтому **любой старт переводится в любой другой** вместе со своим двором,
  соседями и расстояниями — равнозначность позиций следует из построения, а не из подбора
  (`mapFairness.test.ts` проверяет это по графу). Связи заданы **шаблоном в индексах**, не
  геометрией: прежний relative-neighbourhood граф выводил рёбра из координат, и округление
  до целых пикселей после поворота ломало симметрию. Соседние по геометрии узлы связаны не
  всегда — маршрутная сеть это граф, а не «всё со всем поблизости». Нейтральные миры несут
  только два типа планет (`crystalline`/`fortress_world`): типы дают разброс производства
  −0.25…+0.45, и разные наборы у секторов вернули бы стартовый перекос — возврат остальных
  шести отдельным кирпичом (BAL-8). Прежняя раскладка — квадратная сетка 11×11 со стартами по
  периметру — была зеркальной по обеим осям, но зеркало уравнивает противоположные клетки, а
  не клетки разного рода: середина стороны и угол структурно различны, и замер показывал у
  одного старта 77–80% побед. Победа по очкам — **1100** (`SCORE_LIMIT`, прототип
  переопределяет дефолт ядра 600).
...
**Стартовый перекос закрыт картой-«колесом» (BAL-1)** — раскладка описана выше, в разделе про
карту. Замер на обеих семьях: побед по старту **77–80% → 52–53%**, очки/старт **+62…83% → 0–2%**
(1496/1496 и 1513/1479). Заодно просел snowball — **91–92% → 71–75%**: пятая часть «снежного
кома» была форой старта, лидером середины часто оказывался тот, кому досталась жирная позиция.
Фракционный отрыв по очкам сжался до +2…7% при 54–56% побед у azure — на грани шума. Что
осталось: 84% территории по-прежнему меняет хозяина прилётом, snowball 71–75%, кредиты
декоративны, дерево технологий — очередь по цене; исходов `elimination` на равнозначных стартах
не осталось вовсе (все 300 матчей доживают до `timeout`). Про счёт AI-BAL-2 показал главное:
`scoreValue` есть и у зданий, и у провинций, так что любая постройка приближает победу — остаток
снежного кома живёт в СЧЁТЕ, а не в бою.
