...
describe('главы кампании — запас не убывает с номером главы', () => {
  it('запас дополнительных задач не убывает; сюжетное спасение героя идёт отдельно', () => {
    // Спасение учёного (2026-09-26) — отдельная сюжетная задача первой главы.
    // Старый запас дополнительных задач по-прежнему не убывает: 7 → 7 → 8.
    const pools = Array.from(
      { length: PVE_MISSION_COUNT },
      (_, i) => pveChapter(i).objectives.filter((o) => o.kind !== 'recruit').length,
    );
