 ...
 *  слот 2 (глубокий тупик в трёх прыжках). Критерий равноценности выведен двумя замерами:
 *
 *   1. Пары равняли по сумме `productionBonus` и сырому металлу (BAL-9). Провал: 600 матчей
 *      дали секторам с `crystalline` **340 побед против 258**. Причина — бонус и добыча
 *      МНОЖАТСЯ, поэтому равнять их порознь бессмысленно; у проигравшей пары эффективный
 *      металл `metal × (1 + productionBonus)` был 17.95 против 22.25, то есть −19%.
 *   2. Пары равняли по СУММЕ эффективного металла и обороны (22.15 против 22.25, +0.15 у
 *      обеих). Стало заметно лучше, но перекос остался: 900 матчей на трёх семьях дали
 *      478 побед против 417 и +4% очков. Причина позиционная — у эталона весь металл сидит
 *      в БЛИЖНЕМ мире (18.85 из 22.25), который берут первым и держат весь матч, а у второй
 *      пары он был размазан, и компенсация лежала в дальнем тупике, до которого доходят
 *      поздно и не всегда.
 *
 *  Отсюда действующее правило: равняются СЛОТЫ, а не пары. Ближний мир ↔ ближний
 *  (эффективный металл 18.85 против 19.20, оборона −0.25 у обоих), дальний ↔ дальний
 *  (3.40 против 3.15, оборона +0.40 у обоих). Под это подстроены `irradiated` и
 *  `relic_world` — см. их комментарии в `prototypeData.ts`.
 *
 *  Прочие ресурсы (кредиты, энергия, еда) в критерий НЕ входят намеренно: пока они ничего не
 *  ограничивают (BAL-3, кредитов 64k к 11-му дню при нулевых arrears), богатство ими не даёт
 *  преимущества. Это же и ограничивает разнообразие: пока ограничивает только металл, два
 *  «разных» мира обязаны совпадать по нему в каждом слоте и различаться лишь тем, что ничего
 *  не решает. Оживёт BAL-3 — различия станут содержательными, и это увидит тот же замер. */
