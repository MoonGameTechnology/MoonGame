...
decisions/       ОБЩИЕ решения обоих клиентов (`decisions/README.md`): чистая функция + типы + тесты, ни DOM, ни сети. Заведена, потому что вынесенные REFM-модули легли внутрь `prototype/src` и новому клиенту недоступны (после MIG-5 там остаётся 209 модулей, из них 129 прошли бы сторожа как есть — чистые, с тестом рядом и без импорта соседа) — он пишет свои `welcomeScreen`/`matchHud`/`mapRender`/`camera`. Устроена как `/localization`: корневая папка, оба клиента тянут относительным путём, конфигурации сборки не нужно. Явные упоминания — `vitest.config.ts` (include тестов), `prototype/tsconfig.json` (include типов), `prototype/src/i18n.test.ts` (папка в списке потребителей локализации). Контракт папки машинный — `shared.test.ts` читает КАТАЛОГ и держит три правила: из `/decisions` не импортируют `prototype/` (иначе решение молча перестаёт быть общим), исходники чистые, у каждого решения есть тест рядом; все три проверены мутацией. Жильцов **36** (было 5): MIG-1 перевёз цепочку сетевого входа целиком (19 модулей — адрес сервера, вход, обзор матчей, обмен сессии на место, дозвон, приветствие, жизнь сокета, маршруты приказа и отказа), MIG-2/3 добавили имя отказа и имя игровых данных, MIG-4 перевёз цепочку «приказ выделенному флоту» (10 модулей — выделение, что следует из тапа, штурм/слияние/деление/война перед приказом/предложения мира, маршрут на карте; `panelSelect` остался, его тест строит партию через `newGame` прототипа), MIG-5 — ВСЕ строители приказов (`actions.ts`: конверт `Action` для 48 типов плюс `canTraverse`; у файла появился тест, которого не было, — каждый строитель сверяется с настоящей схемой гейта `actionPayloadSchemas`, 70 тестов). Умолчание прежнее — старое переезжает, когда его и так трогают; разом везут только связную цепочку, которой второму клиенту не хватает сейчас, и только при чистоте, тестах и НЕИЗМЕННОМ числе тестов в гейте (условия — `decisions/README.md`)
