 ...
 *
 * МОДИФИКАТОРЫ ПОСТЕРА — ОДНИ НА ОБЕ ПОВЕРХНОСТИ (REFM-129). Силуэт был общим с
 * самого начала, а вот два модификатора поверх него — размер и гало — карта считала
 * своими числами:
 *
 * 1. **Размер S/M/L — ОДНА таблица.** Панель брала `L 1 · M 0.84 · S 0.68`, маркер
 *    карты — `L 1 · M 0.8 · S 0.62`, то есть один и тот же разведчик был в панели
 *    заметно крупнее, чем на карте. Постер обещает «размер = hp» ОДНИМ языком: игрок
 *    учится читать размер на тайлах и применяет это к карте. Две таблицы ломают ровно
 *    это обещание, и молча — картинка не «падает», просто перестаёт быть шкалой.
 * 2. **Гало — ОДНО правило.** «Есть щит ИЛИ флагман» стояло дважды, слово в слово.
 *    Убери кто-нибудь флагмана из одной копии — и корабль с орбитой в панели остался
 *    бы без неё на карте, причём заметить это можно только сравнив два экрана.
 * 3. **Радиус кольца у поверхностей РАЗНЫЙ — и это НЕ дубль.** В панели бокс тайла
 *    фиксирован, поэтому кольцо там постоянного радиуса: иначе сетка тайлов «дышала»
 *    бы от размера корабля. На карте бокса нет, и кольцо обязано следовать за
 *    силуэтом, то есть масштабироваться вместе с ним. Единая тут — причина, а не
 *    число; сводить их в одну константу было бы ошибкой в обратную сторону.
 */
