...
- **Карта — «колесо» из десяти секторов (`map.ts`, BAL-1 → BAL-9):** 121 провинция — ровно **30
  ...
  сектора). Третьей осью различия пробовали типы планет — вторая пара подбиралась по равной сумме
  `productionBonus` и близкой добыче металла и всё равно проиграла замеру (340 побед против 258),
  потому что ресурсы не равноценны: кредиты `relic_world` не ограничивают ничего (BAL-3), а
  `crystalline`+`fortress_world` дают ещё и +0.2 обороны. Типы вернулись к общей паре. Сумма
