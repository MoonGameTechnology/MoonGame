...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5695 зелёных** (61 skip, 429 файлов).
...
> **Что означают оставшиеся 99 ПОСЛЕ шага B.** Это уже не «дрейф двух живых каталогов»:
...
информационные сканеры краснеют штатно (чужие CVE в прод-образах caddy/postgres нашим
кодом не чинятся; по caddy апстрим не пересобирал с 2026-06-22, поэтому SEC-30 перевёл
план с «ждём пересборки» на собственную сборку — SEC-31), и требование строгого `CLEAN`
остановило бы авто-постановку для всех PR разом. Гейт этим
