...
effects])` (34 модуля), тик в реальном
...
`DEV_MODULES` — 32): экономика + рынок, карта/движение/перехват, типы
