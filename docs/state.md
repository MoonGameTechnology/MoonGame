...
packages/client/src/  holoDraw.ts, holoSphere.ts (каркасные атласы), territory.ts, provinceSelection.ts, spaceBackdrop.ts, art/deep-space.webp (общий рендер карты)
...
- **UI — тактический пульт (DEFCON-вайб):** векторно-каркасный стиль на чёрном.
  ...
  - **Телефонный тактический интерфейс — первый этап:** `mobileHud.ts` переносит
    ...
    сохранены. Отдельная адаптация стратегических экранов — следующий этап.
...
- **Bytro-карточка армии + визуальная система кораблей (2026-07-22, постер владельца):**
  `prototype/src/unitGlyphs.ts` — 5 архетипов-силуэтов из полей unit-def (флагман
  `hero` > рой `faction=swarm` > транспорт `cargo≥8` > скаут
