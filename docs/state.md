...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5669 зелёных** (61 skip, 428 файлов).
...
- **Челноки-носители** (shuttles-roadmap SQ-1.1→4.1; переименовано из «эскадрилий» в
  SHU-0.1 — трейт `shuttle`, юнит `interceptor`, действия `shuttle.*`, манифест 9→10.
  МОДЕЛЬ МЕНЯЕТСЯ: заказ владельца 2026-09-08 переносит челноки в космопорт и делает удар
  «прилетел → ударил → сразу домой»; описанное ниже — ТЕКУЩЕЕ поведение до SHU-1.x):
  `interceptor` +
