...
/** Real touch zoom regression in normal and software-preferred modes. Run after pnpm run prototype.
  ...
  for (const profile of ['alpha', 'player'])
    for (const compatibility of [false, true]) {
      const page = await browser.newPage({
        ...
        hasTouch: true,
        userAgent:
          'Mozilla/5.0 (Linux; Android 15; K; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/151.0.7922.199 Mobile Safari/537.36',
      });
      const errors = [];
      ...
      await page.addInitScript((enabled) => {
        ...
        localStorage.setItem('void.canvasCompatibility', enabled ? '1' : '0');
      }, compatibility);
      ...
      const pendingReport = JSON.parse(await page.evaluate(() => window.__zoomTest.report()));
      assert.equal(pendingReport.compatibility.active, compatibility);
      assert.equal(pendingReport.compatibility.requested, !compatibility);
      assert.equal(pendingReport.compatibility.restartPending, true);
      await page.locator('#set-render-report').click();
      const report = JSON.parse(await page.locator('#set-render-report-text').inputValue());
      assert.equal(report.compatibility.restartPending, true);
      assert.equal(report.canvas.attributes.willReadFrequently, compatibility);
      assert.equal(report.frameErrors, 0);
      ...
      for (const viewport of [
        ...
        for (const distance of [96, 128, 160, 192, 160, 128, 96, 80]) {
          ...
          near(
            ((mid.y - before.camera.y) / 4) * scale + current.camera.y,
            mid.y,
            'vertical anchor',
          );
        ...
        console.log(
          `PASS ${profile} compatible=${compatibility} ${viewport.width}x${viewport.height}: anchored pinch, cap pan, one-finger handoff, cancel and bounded rebakes`,
        );
      }
      const finalReport = JSON.parse(await page.evaluate(() => window.__zoomTest.report()));
      assert.equal(finalReport.canvas.attributes.willReadFrequently, compatibility);
      assert.equal(finalReport.background.attributes.willReadFrequently, compatibility);
      assert.equal(finalReport.frameErrors, 0);
      await page.close();
    }
} finally {
  await browser.close();
