...
отклоняется. Тест.
**Кто ещё этого ждёт:** фильтр «Режим» и подпись режима на карточке в браузере
матчей (блок `BRW` в `backlog.md`) заперты именно здесь — режима сегодня нет ни в
`MatchConfig`, ни в `MatchSummary`.
