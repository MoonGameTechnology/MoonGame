...
Статус на 12 сентября: **актуальная векторная проекция прошла полный гейт и
проверку основных взаимодействий в Chromium 1400×960**. Кандидат основан на
`main` (`8c1dacbd`, влитый PR #971). Текущие результаты — в последней секции;
проверки и снимки от 10–11 сентября ниже сохранены как история.
...
Карточка флота и исходный ряд приказов объединены общей рамкой. Перемещение
за заголовок и стрелками меняет только оконную геометрию; положение сохраняется
при pan/zoom, переключении состава/сводки, прицеливании и повторном открытии.
Сохранены исходные обработчики кнопок, порядок Back/Escape и отдельная геометрия чата.

Проверка Chromium: общий контейнер, отсутствие второй шапки, перетаскивание,
состав/сводка, панорама/зум, прицел движения и отмена — без изменения снимка
игрового состояния и без ошибок JavaScript. Проверены освобождение указателя
и защита от ложного клика по фону после перетаскивания диалога.

Новая кромка использует координаты мира и существующий визуальный таймер.
Тестами проверены точные перенос/масштаб, узкая полоса края, остановка на том же
времени и отключение широкого свечения. Ровная основная рамка и тихая волна сохранены.

Полный гейт, dev/player/admin и UI smoke зелёные. CPU p95 в заглушенном Canvas:
idle 1,15 мс, pan 6,63 мс, zoom 6,56 мс; этот замер не оценивает GPU устройства.

![Единое окно флота](docs/previews/holographic-fleet-window.jpg)
