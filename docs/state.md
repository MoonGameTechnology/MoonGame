...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5809 зелёных** (70 skip, 434 файла; с `DATABASE_URL` — 5879 без пропусков). Пропуски — тесты durable-пути и учение бэкапов, которым нужна база; в CI база есть, но нет пакета `age`, поэтому там 5878 и пропущен один тест — реальный криптокруг (проводку шифрования CI проверяет заглушкой, см. §10).
...
- **Челноки** (shuttles-roadmap; переименовано из «эскадрилий» в SHU-0.1 — трейт
  ...
  **ИНТЕРФЕЙС (SHU-3.1, половина):** вкладка «Крылья» панели мира показывает АНГАР ПОРТА
