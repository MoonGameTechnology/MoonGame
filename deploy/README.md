...
Бэкап снимает `deploy/backup.sh`, восстанавливает `deploy/restore.sh`. Оба — файлы
репозитория, поэтому приезжают на хост обычным `moongame update`; отдельно
устанавливать и обновлять их не нужно.

**Ручного конвейера `pg_dump | gzip > file` здесь больше нет, и это не косметика.**
Учение 2026-09-09 (настоящий кластер, настоящая схема, 26 таблиц) показало на нём два
тихих отказа: `pg_dump` к недоступной базе даёт код возврата **0** и корректный
gzip-файл в 20 байт (последним в конвейере стоит успешный `gzip`), а восстановление
такого обрезанного файла через `psql < dump` уничтожает живую базу — **тоже с кодом
возврата 0**: из 26 таблиц осталось 10. Обрыв не ловится ни кодом возврата, ни
контрольной суммой, ни `ON_ERROR_STOP` — обрезанный дамп не содержит ошибок, он
выполняет `DROP` всех таблиц и просто кончается. Скрипты ловят его по хвостовому
маркеру `pg_dump`; подробности — в шапках самих файлов.

### Ключ шифрования (один раз)

Дамп несёт `users.pass_hash` и почту для восстановления доступа, поэтому открытым на
хранение он не выносится: без настроенного получателя `backup.sh` **отказывает**, а не
кладёт открытый файл.

На уже развёрнутой машине `age` ставится руками — установщик до неё не доезжает
(`apt install age`); свежие установки получают пакет сами.

```bash
age-keygen -o /root/void-backup.key       # приватный ключ — ХРАНИТЬ ВНЕ сервера
# в выводе строка «Public key: age1...» — это и есть получатель
```

На бэкап-хосте нужен только **публичный** ключ: захват сервера не раскрывает архивы.
Приватный ключ хранится отдельно — если он лежит рядом с бэкапами, шифрование
бессмысленно.

```bash
# /etc/default/void-backup — читается кроном
BACKUP_DIR=/var/backups/void
...
```bash
set -a; . /etc/default/void-backup; set +a
/opt/moongame/deploy/backup.sh
```

Автоматический бэкап (crontab) — **ежедневно в 04:00**, что и задаёт RPO (см. ниже):

```bash
0 4 * * * set -a; . /etc/default/void-backup; set +a; /opt/moongame/deploy/backup.sh >> /var/log/void-backup.log 2>&1
```

### Восстановить

```bash
BACKUP_AGE_IDENTITY=/root/void-backup.key \
  /opt/moongame/deploy/restore.sh /var/backups/void/void-20260909T040000Z.sql.gz.age
```

Дамп снимается с `--clean --if-exists`, поэтому ложится **поверх** существующей базы —
пересоздавать volume не нужно. Применяется одной транзакцией с `ON_ERROR_STOP=1`: если
дамп не лёг, база откатывается целиком, а не остаётся наполовину восстановленной.
Восстановление в пустой том тоже работает — подними стек, дождись готовности postgres и
запусти ту же команду.

### RTO / RPO

Измерено на учении, синтетическая нагрузка формы боевого `GameState` (много однотипных
объектов флотов и планет), локальный кластер PostgreSQL 16:

| Размер БД | Архив  | Бэкап  | **Восстановление** |
| --------- | ------ | ------ | ------------------ |
| 30 МБ     | 764 КБ | 1.9 с  | 3.3 с              |
| 223 МБ    | 7.4 МБ | 19 с   | 35 с               |
| 1080 МБ   | 37 МБ  | 83 с   | **181 с**          |

- **RTO по базе ≈ 3 минуты на гигабайт** — зависимость близка к линейной. Это время
  только на восстановление данных; полный RTO сервиса добавляет обнаружение аварии и
  подъём стека.
- **RPO = 24 часа** при суточном кроне: теряются изменения с момента последнего
  бэкапа. Меньший RPO требует PITR (архивация WAL) — его ещё нет, см. SE-3.4.

Учение не разовое: `deploy/backup.test.mjs` повторяет его на каждом прогоне CI —
снимает бэкап настоящим `pg_dump`, **дропает базу целиком** и сверяет отпечаток каждой
таблицы после восстановления.
