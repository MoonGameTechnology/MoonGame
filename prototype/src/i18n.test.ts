...
/** Модули ядра — источник кодов отказа, которые видит игрок (LOC-7). Читается КАТАЛОГ,
 *  а не список: новый модуль попадает под гейт сам, без правки теста. */
const coreModuleFiles = (): string[] =>
  readdirSync(path.join(repoRoot, 'packages/shared-core/src/modules'))
    .filter((f) => f.endsWith('.ts') && !f.endsWith('.test.ts'))
    .map((f) => `packages/shared-core/src/modules/${f}`);
...
/** Шипнутый каталог — ВТОРОЙ источник живых ключей, наравне с исходниками (CONV-12c).
 *  Часть полей контента (`description` технологий, способностей, фракций…) хранит не
 *  текст, а КЛЮЧ: потребитель зовёт `t(def.description)`. Пока каталог был TS-литералом
 *  `prototypeData.ts`, эти ключи попадали в учёт вместе с остальными исходниками; после
 *  переезда на `data/*.json` их надо читать оттуда, иначе 79 живых ключей прочтутся как
 *  мёртвые. Значения в JSON в двойных кавычках, поэтому общий разбор литералов их видит. */
const catalogFiles = (): string[] =>
  readdirSync(path.join(repoRoot, 'data'))
 ...
 *  `packages/client`, сервер И прототип — каталог с CONV-12b один. Пары
 ...
 *  тихо зарастает и гейт превращается в декорацию.
 *  Заведён 2026-08-06 (AUD-4) на 24 записях; фракции (6) сняты в AUD-14 — у них имена
 *  совпадали в обоих каталогах, так что решать было нечего. Остаток (14) держала
 *  развилка CONV-12: у прототипа и канона для одних и тех же зданий были РАЗНЫЕ имена
 *  (`Metal Mine` против `Metal Extractor I`), и «просто завести ключ» требовало сперва
 *  решить, какое имя каноническое.
 *  2026-09-08, CONV-12a — список ПУСТ. Развилка решена: канон принял имена прототипа
 *  (шесть записей закрылись сами — ключи под эти имена давно есть), а остальные восемь
 *  оказались не решением, а просто ненаписанным текстом, и он написан. Список остаётся
 *  в коде намеренно: он — механизм, а не запись. Первая же сущность в `data/*.json`
 *  без ключа `data.*` уронит тест, и допустить её сюда можно будет только осознанно. */
...
describe('локализация — ключи', () => {
  ...
  it('ключи, лежащие в полях игрового каталога, заведены в локали', () => {
    // Каталог хранит в части полей (`description` у технологий, способностей, фракций)
    // не текст, а КЛЮЧ — он уходит в `t()` переменной, поэтому разбор литералов его не
    // видит. Опечатка здесь доехала бы до игрока сырым ключом: ровно так на экране
    // совета учёных светилось `sci.overseer.name`.
  ...
  it('каждый отказ ЯДРА назван словами в обеих локалях', () => {
    // `errText()` (`decisions/refusalText.ts`) ВЫВОДИТ ключ из кода отказа, а не берёт
    // его из таблицы. Это намеренно — новый код требует ровно одной правки, текста в
    // локали, — но плата в том, что забытая запись ничего не ломает: игрок просто видит
    // сам код словами («hangar full», «lane busy»). Так и жило: из 116 кодов, которыми
    // модули ядра отбивают приказ, текст был у 51 (LOC-7; FORT-1.4 закрыл до этого
    // крепость и поднял счёт с 49).
    //
    // Список кодов не переписан сюда руками, а ЧИТАЕТСЯ из исходников: новый
    // `h.reject()` без текста обязан валить гейт, а не ждать, пока его увидят на экране.
    // Мерка — именно `reject('E_*')`: подстрочный поиск `E_[A-Z0-9_]+` ловит хвосты
    // обычных констант (`CHANCE_MIN` → «E_MIN») и намеряет вдвое больше несуществующего.
    const codes = new Set<string>();
    for (const file of coreModuleFiles())
      for (const m of read(file).matchAll(/reject\('(E_[A-Z0-9_]+)'\)/g)) codes.add(m[1]!);
    expect(codes.size).toBeGreaterThan(100); // разбор не должен молча опустеть
    // Отказы, которые модуль не бросает сам, а получает из хука: `construction.requirement`
    // (`technology.ts`) запирает постройку до исследования — им заперты и форт, и
    // орбитальное ПКО (ORB-1). Хук отдаёт код значением, поэтому разбор его не видит.
  ...
  it('каждый ключ локали используется (нет осиротевших переводов)', () => {
    ...
    const hay = [
      ...srcFiles().map(read),
      ...catalogFiles().map(read),
      read('prototype/build.mjs'),
    ].join('\n');
  ...
  it('у каждого юнита есть переведённое имя', () => {
    ...
    // CONV-12c: раньше id доставались РЕГУЛЯРКОЙ из исходника `prototypeData.ts` —
    // единственного тогда каталога прототипа. Файл снесён, и разбирать текст больше
    // незачем: список берётся из загруженного каталога напрямую, что и точнее
    // (регулярка молча промахнулась бы на смене форматирования), и честнее — тест
    // смотрит на то, чем игра реально играет.
    const ids = Object.keys(data.units);
    expect(ids.length).toBeGreaterThan(5); // каталог не должен молча опустеть
  ...
  //
  // CONV-12c: рядом стоял ЧЕТВЁРТЫЙ тест — «у каждого имени в каталоге ПРОТОТИПА есть
  // ключ `data.*`». Он существовал ровно потому, что каталогов было два: тот проверял
  // рукописный `prototypeData.ts`, эти — бандл. Каталог теперь один, и тот тест стал
  // не просто дублем, а ОСЛАБЛЕННЫМ дублем: он смотрел пять таблиц вместо всех и
  // только локаль `ru` вместо обеих. Снят вместе со вторым каталогом — держать рядом
  // две проверки одного факта хуже, чем одну, тем более когда слабая зелёная.
