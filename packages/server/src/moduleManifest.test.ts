 ...
 *  знание части.
 *  38 — AUD-27: членство и порядок не тронуты, `shuttle` считает перезарядку от
 *  накопленного времени; список перезакреплён без правок.
 *  39 — разрывы сети Роя: членство и порядок не тронуты, сменились ПРАВИЛА и форма
 *  `swarmNet` (`linked`/`cut`) и `swarmJournal`; список перезакреплён без правок.
 *  40 — урон по роду войск: членство и порядок не тронуты, сменились ПРАВИЛА наземного боя
 *  (`combat` 2.3.0 — залп под состав цели, раздельно по пехоте и технике); список
 *  перезакреплён без правок.
 *  41 — PVR-6.20: членство и порядок не тронуты, форму изменило ДОБАВЛЕНИЕ боевого счёта
 *  `PveState.tally` (как `boons` в 22); список перезакреплён без единой правки.
 *  42 — PVR-6.21: членство и порядок не тронуты, сменились ПРАВИЛА зрения и форма
 *  `GameState.sight` (обзор по виду провинции, `byKind`); список перезакреплён без правок.
 *  43 — PVR-4.7: членство и порядок не тронуты, сменились ПРАВИЛА штурма, героя и
 *  выводка — Левиафан, босс с последней волной; список перезакреплён без правок.) */
const PINNED_FOR_VERSION = '44';
