...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5113 зелёных** (58 skip, 390 файлов).
...
pnpm run selfplay 300 base   # батч AI-vs-AI на реальном ядре (баланс), сессия 7 дней
...
мёртвого контента 21 → 8 позиций), AI-BAL-7 — **выход из проигранного боя** (`aiRetreat.ts`:
выходов 0 → 6134/6313 за 300 матчей, флотов погибло −20%/−18%). Вне охвата осталось то, чего
ещё нет в самой игре: свободный вылет эскадрилий не достроен (`squadron.strike` требует
`fleet.homeBase`, которое не выставляет ни один модуль), поэтому носитель боту не нужен —
`backlog.md`, блок AI-BAL. Там же остаток самого AI-BAL-7: `fleet.bombard`/`fleet.barrage` и
`fleet.split` бот по-прежнему не зовёт, и каждое из них — отдельный замер (три поведения в
одном прогоне дают неатрибутируемый сдвиг).
...
**ВАЖНАЯ ОГОВОРКА К ЭТИМ ЦИФРАМ (AI-BAL-7): все они измерены прибором, в котором бот НЕ умел
выходить из боя, и потому РАЗРЫВ В НИХ ЗАНИЖЕН.** Пока флот дрался до нуля, взаимное
уничтожение работало уравнителем: сильный доигрывал проигранные драки и стирал собственное
преимущество. Стоило дать боту `fleet.retreat` — и разрыв на тех же семьях сидов вырос
**344/337 → 390/396**, а прирост отстающего во второй половине просел (116/114 → 96/104).
Сама правка BAL-4 от этого не отменяется (темп раздела карты она растянула), но сравнивать
её «до/после» с более поздними замерами нельзя — прибор между ними сменился. Актуальная
база разрыва — **390/396**.
