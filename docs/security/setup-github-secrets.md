...
**Используемые секреты — один (SEC-15):**

| Имя | Кто читает | Что будет без него |
|---|---|---|
| `NVD_API_KEY` | джоба `dependency-check` в `security.yml` | джоба не запускается, а в сводном отчёте помечается «⚠️ НЕ подтверждён» — не тихий зелёный |

Ключ бесплатно запрашивается на nvd.nist.gov (Request an API Key) и снимает жёсткий
rate-limit на выкачку фида; сам по себе он не даёт доступа ни к чему приватному, но это
всё равно секрет: только `Settings → Secrets and variables → Actions`, никогда в репозиторий
(попадёт в коммит — уронит блокирующую джобу Gitleaks, что и правильно).

Прочих внешних токенов (AWS, Docker Hub, etc) проект **НЕ использует**, но если будут:
