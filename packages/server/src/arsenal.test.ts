 ...
 *     молча. Наземные войска и герой сюда попали не по недосмотру: арсенал по замыслу
 *     (`docs/arsenal-roadmap.md`) — про «корпуса КОРАБЛЕЙ, модули, фитинги героев», а
 *     гейт не различает домен. Закрывать эту разницу — решение владельца (правкой гейта
 *     или выдачей), и до него список обязан оставаться видимым, а не подразумеваемым.
 */
describe('достижимость содержимого на гейтированном месте', () => {
  ...
  /** Наземный юнит и герой — не корабли; см. шапку, это открытый вопрос владельца. */
  ...
  it('каждый КОРАБЛЬ можно получить — иначе он мёртвый контент', () => {
    ...
    const dead = Object.keys(data.units)
  ...
  it('хотя бы один челнок есть С ПЕРВОЙ СЕКУНДЫ — иначе ангар порта пуст и наполнить нечем', () => {
