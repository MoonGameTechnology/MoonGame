...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5824 зелёных** (62 skip, 435 файлов; с `DATABASE_URL` — 5886 без пропусков: все пропуски — тесты durable-пути, которым нужна база).
