...
Итого **1022 ключа**. Записи в локалях разложены по доменным секциям и отсортированы
...
локалью. Оставшиеся **310** вызовов (`main.ts` 300, `sandbox.ts` 10) ещё держит **мост**
`/localization/legacy/en.ts` (старый msgid = русская строка, 528 записей); мост только
