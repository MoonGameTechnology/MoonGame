...
describe('game data schema (docs/architecture.md §2)', () => {
  it('validates the shipped data bundle', () => {
    ...
    expect(data.version).toBe('0.1.52'); // четыре универсальных отсека усиленного крейсера (SM-0.9) поверх 0.1.51: усиленный крейсер поверх 0.1.50: авианосец и десантный корабль слиты в «Носитель» поверх 0.1.49: радары забега и полигона ×1,5 вместо ×2,5 (PVR-6.33) поверх 0.1.48 (ремонтный ангар, SHU-5.4)
    ...
    // VET-6: и корпус ветерана — та же причина. Действуют обе ставки только там, где их
    // включил хост (Sector Zero), но обнулённая в данных половина пропала бы и оттуда.
    expect(data.veteran.hullPerBattle).toBeGreaterThan(0);
  ...
  it('исследование запирает ровно три вещи — и список закрыт намеренно (CONV-12)', () => {
    ...
    //
    // FORT-5.10 (2026-09-22) — СЕДЬМАЯ строка, и по той же мерке дешёвая: щитовой контур
    // крепости стоит только на ней (`onlyOn`), заведён вместе с этим кирпичом и в минуту
    // первую недоступен по определению — до него надо изучить саму крепость. У замеров
    ...
    // тут нечего, а ветка получает обещанную решением 12 глубину: крепость → её постройки.
    //
    // FORT-5.1 (2026-09-16) — ПЯТАЯ строка, и по мерке этого сторожа она из дешёвых:
    // запирает не «доступное с первой минуты», а ядро космической крепости, которое
    // руками не строится ВООБЩЕ (`onlyOn: []`) и появляется только вместе с самой
    // крепостью. Гейт стоит на `station.deploy`, а та механика стала достижимой лишь
    // накануне (FORT-0.2) — то есть отбирать у замеров BAL тут нечего. Узел ярус 2 за
    // орбитальной обороной: крепость это шаг ПОСЛЕ того, как игрок научился держать
    // орбиту, а не вместо него.
    //
