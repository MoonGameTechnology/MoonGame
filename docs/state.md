...
- **Слоты под скиллы: владеть ≠ носить (HPR-1.2, модуль `hero` v1.1.0).** `Hero.abilities`
  ...
  явно (`matchSetup.ts`).
- **Железо героя — обычные модули корабля (HPR-1.5.1, данные).** У юнита `hero` теперь есть
  типизированные отсеки `{weapon:1, defense:1, utility:1}` — форма крейсера, одна на всех
  героев (архетипы делят один корпус). Кода это не потребовало: `effectiveStats`/`canEquip`
  живые с `SHIP-3`, и модуль, поставленный на корабль героя, сразу меняет статы, которые
  читает бой. `domain` у `UnitDef` дефолтится в `space`, поэтому `allowed.domain: space`
  проходит; `radar_module` с `allowed.units: ['sensor_frigate']` на корпус героя по-прежнему
  не встаёт. Две оси героя развело окончательно: бюджет ЖЕЛЕЗА даёт КОРПУС (одинаково у
  всех, у основного героя будет +1 отсек), бюджет УМЕНИЙ — РЕДКОСТЬ. Хранение набора на
  герое и перенос через смерть — `HPR-1.5.2`, ещё не сделано.
