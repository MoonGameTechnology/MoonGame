...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5637 зелёных** (61 skip, 428 файлов).
...
- **`fleet.split {fleetId, take[], takeLanding?}`** — отделяет выбранные корабли в новый
  ...
  **Отделение ЧИСТО эскадрильного состава даёт крыло (SQ-1.1):** новый флот получает
  `homeBase` (носитель), и только это отпирает свободный полёт `squadronModule`
  (`squadron.strike`/`return` мимо графа линий) — без `homeBase` удар отвергается
  `E_NOT_SQUADRON`, и весь свободный вылет был недостижимым кодом.
  Два условия здесь неочевидны и оба проверены тестами (`squadron.test.ts`).
  **Крылом становится только ЧИСТЫЙ squadron-состав, а не «хотя бы один истребитель»:**
  свободный полёт уносит ВЕСЬ флот, поэтому «хотя бы один» позволял увести крейсер мимо
  графа линий, подцепив его к отделяемым эскадрильям (`squadronTake` в `state/squadron.ts`
  определяет крыло ровно как squadron-стеки). **Второй координаты крылу не выдают:**
  `squadron.strike` берёт начало полёта как `freePosition ?? позиция location`, а у
  ...
  ходом по лейну, и крыло, уведённое `fleet.move`, для всей эскадрильной логики
...
> строился мгновенно. Там же `slots` у `fighter_squadron`/`strike_carrier` и `upkeep` у
...
drop_infantry, tank(cargoSize 1), heavy_infantry, special_forces, hero, fighter_squadron,
...
Вне охвата остался носитель: на момент замера свободный вылет эскадрилий был недостижим
(`squadron.strike` требует `fleet.homeBase`, которого не выставлял ни один модуль), поэтому
боту он был не нужен — `backlog.md`, блок AI-BAL. **С SQ-1.1 вылет ожил** (см. §5, `squadron`),
