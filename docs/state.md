...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5611 зелёных** (61 skip, 428 файлов).
...
## 6. Данные (`data/*.json`, версия `0.1.13`)
...
> `econplaytest.mjs`) читают ДРУГОЙ каталог — `prototype/src/prototypeData.ts` (1320 строк).
...
- **Герои — полная новая модель в прототипе:** 5 hero-каталогов инлайн в данных
  ...
  **все не-встроенные эффекты имеют провайдеры → «скоро» не осталось**), дерево
  `hero.skill.unlock`, фиттинги `hero.fit`. Кастуемость — `HERO_CASTABLE` (built-ins +
