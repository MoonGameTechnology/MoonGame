...
Итого **460 ключей**. Статичный узел в разметке ПУСТ, ключ стоит в ЗНАЧЕНИИ атрибута
...
локалью. Оставшиеся **872** вызова (`main.ts` 858, `sandbox.ts` 10,
`spotlightDom.ts` 4) ещё держит **мост** `/localization/legacy/en.ts` (старый msgid =
русская строка, 992 записи); мост только сокращается, добавлять в него нельзя —
