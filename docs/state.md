...
Итого **302 ключа**. Статичный узел в разметке ПУСТ, ключ стоит в ЗНАЧЕНИИ атрибута
...
локалью. Оставшиеся **1044** вызова (`main.ts` 1030, `sandbox.ts` 10,
`spotlightDom.ts` 4) ещё держит **мост** `/localization/legacy/en.ts` (старый msgid =
русская строка, 1104 записи); мост только сокращается, добавлять в него нельзя —
домиграция это кирпич `LOC-2`. Гейт — `prototype/src/i18n.test.ts` (8 тестов: паритет
ru/en, ключ из кода заведён, ключ локали не осиротел, в EN нет кириллицы, у каждого
юнита есть имя, статичная разметка на ключах и без старой формы, мост не растёт).
