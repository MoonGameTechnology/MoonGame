...
- **Ростер по типу провинции (province-centric):** `sectorKinds[kind].allowedBuildings`
  ...
  Так у каждого типа свои постройки: **планета** — всё; **`dead_world`** — только
  `metal_station`; **`void_station`** — верфь/космопорт/радар/форт/электростанция/фабрикатор
  **и орбитальное ПКО** (без шахт/казарм): крепость несёт орбитальный слой, значит и зубы
  против флота над собой (ORB-1). Астероид и туманность роспись не сужают вовсе.
