...
describe('shipped maps resolve against the shipped catalogue', () => {
  ...
  it('the maps exercise the terrain catalogue, not just nebula and asteroid', () => {
    ...
    expect([...terrains].sort()).toEqual([
      'asteroid_cluster',
      'asteroid_field',
      // Втора́я глава Сектора Зеро (`pve-2`) расширила набор осознанно: просторная
      // местность держит перекрёстки, обломки и мёртвая система — узкие узлы.
      'deep_void',
      'dense_nebula',
      'depleted_system',
      'derelict_graveyard',
    ...
    // `empty` ВЕРНУЛСЯ со второй главой Сектора Зеро, и прежнее объяснение здесь
    // устарело. Оно гласило, что в мозаике пустой узел «перестал быть выразимым»: у
    // точки схода линий нет клетки, значит нет и границы. Верно это было про РАЗВИЛКИ-
    // ЗАПОЛНИТЕЛИ старого стиля (`gate_north`, `core`, `crossing` в `pve-1`) — узлы,
    // существовавшие только чтобы нарисовать путь. С выведенным соседством они не нужны.
    //
    // Но `empty` как ПОЛНОЦЕННАЯ клетка решётки выразим прекрасно, и `pve-2` им
    // пользуется: перекрёсток, сквозь который летают все и который нельзя присвоить.
    // Это содержательный выбор — узел-дорога, а не узел-приз: отвоёвывать надо миры, а
    // не развязки между ними.
