...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5775 зелёных** (70 skip, 436 файлов; с `DATABASE_URL` — 5845 без пропусков). Пропуски — тесты durable-пути и учение бэкапов, которым нужна база; в CI база есть, но нет пакета `age`, поэтому там 5844 и пропущен один тест — реальный криптокруг (проводку шифрования CI проверяет заглушкой, см. §10).
...
- **Челноки** (shuttles-roadmap; переименовано из «эскадрилий» в SHU-0.1 — трейт
  ...
  **ИНТЕРФЕЙС (SHU-3.1, половина):** вкладка «Эскадра» панели мира показывает АНГАР ПОРТА
  ...
  гарантированно пустой, и построенные машины для игрока не существовали. Остаток
  кирпича: на КАРТЕ вылет по-прежнему невидим (ни трассы, ни радиуса прицела, ни вспышки
  перехвата) — игрок видит результат в журнале, но не путь.
