 ...
 * линии, иначе одна и та же группа посчитается дважды — в кораблях и в крыльях. Земля
 * и корабль определяются ОДНИМ признаком — `domain`. Раньше «не корабль» читался по
 * трейту `ground`, а его носят не все наземные: гарнизон крепости, десант и десантник
 * Роя его не имеют, и вкладка «Флот» показывала их «кораблями в гарнизоне», хотя они
 * уже стояли на «Земле» (владелец 2026-09-26: «гарнизон — только наземный юнит»). Так же
 * читает землю ядро (`autoRally`, `fleet.launch`).
 *
...
/** Корабль линии: всё, что не наземное и не крыло. Наземное — по тому же `domain`, что у
 *  {@link isGroundUnit}, поэтому юнит не попадает в две вкладки сразу. */
export function isShipUnit(unit: string, data: GameData): boolean {
  return !isGroundUnit(unit, data) && !isWingUnit(unit, data);
}
