...
Set `DATABASE_URL` to run the same restart phase through the real PostgreSQL match and
receipt stores. The rehearsal writes to a unique match id and removes its rows afterward:

```bash
DATABASE_URL=postgres://void:void@127.0.0.1:5432/void pnpm run rehearsal
```

`PLAYERS` is 2–10. All delay values are non-negative milliseconds. This is deliberately
self-contained by default: it needs neither a white IP nor Docker/Postgres and therefore verifies
the **wire/persistence protocols**. With `DATABASE_URL`, snapshot and receipt restoration also
cross the real PostgreSQL adapters, but the harness still does not emulate an OS-level process
crash or every mechanic's successful game-state setup. Focused module tests remain the source of truth for
...
- **Scheduling:** ✅ a v1 offline scheduler exists — `MatchRoom.tick()`/`msUntilNextEvent()` + the shared single-process wakeup driver (`clockDriver.ts`; both hosts use `startClockDriver` — the prototype host's old inline `setTimeout` driver was removed in NETA2-6) fire due events with nobody connected. ⚠️ a durable, cross-process wake-up (pg-boss, v2 — NOT Redis/BullMQ) is needed for >1 server process.
...
- **Action gate:** ✅ `@void/action-layer` is wired (`MatchRoom.gate`, `GATE=1`): `action.v1` envelopes pass validate → payload schema → authorize → dedup → sequence before the reducer, and bare `action` is refused on a gated room. ✅ the client sends envelopes: `MultiplayerClient` flips to `action.v1` on the `welcome.gated` handshake (strict per-session `clientSeq`, reset on reconnect).
