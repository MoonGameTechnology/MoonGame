...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5852 зелёных** (70 skip, 437 файлов; с `DATABASE_URL` — 5922 без пропусков). Пропуски — тесты durable-пути и учение бэкапов, которым нужна база; в CI база есть, но нет пакета `age`, поэтому там 5921 и пропущен один тест — реальный криптокруг (проводку шифрования CI проверяет заглушкой, см. §10).
