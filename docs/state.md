...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 4861 зелёный** (58 skip, 371 файл).
...
нет** — он грузит этот модуль. Не поехал вместе с четвёркой авто-сбор построенного
(BF-29): он реализован только у прототипа, то есть это пробел КАНОНА, а не дубль —
остался там модулем `autoRally.ts`, заведён кирпичом CONV-10.
...
механикой: авто-сбор построенного (BF-29) реализован ОДИН раз, поэтому остался у
прототипа своим модулем (`autoRally.ts`), а в каноне его нет — заведено кирпичом CONV-10.
...
двенадцать файлов.** Открытым остался CONV-10 — авто-сбор построенного, которого нет в
каноне (пробел, а не дубль).
...
**✅ Этап 1 (ядро) — готово целиком:** **35 модулей** на микроядре (шина/хуки/манифест,
...
**Написано ≠ собрано: ядер ДВА, и они собирают РАЗНОЕ.** 35 модулей лежат в каталоге, но
ни одно ядро не берёт все:

| Сборка | Модулей | Чего нет |
| --- | --: | --- |
| `DEV_MODULES` (канонический сервер, `scenario.ts`) | **31** | `effects`, `espionage`, `heroEffects`, `steward` |
| `MODULES` (ядро прототипа, `protoKernel.ts`) | **35** | `pve`, `station`, `visibility` |

Прототипные 35 — это 32 модуля ядра плюс три СВОИХ (`hunger`, `autoRally`,
`botDiplomacy`), которых в `shared-core` нет вовсе. Практический смысл расхождения:
