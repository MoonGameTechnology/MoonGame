...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5765 зелёных** (62 skip, 433 файла; с `DATABASE_URL` — 5827 без пропусков: все пропуски — тесты durable-пути, которым нужна база).
