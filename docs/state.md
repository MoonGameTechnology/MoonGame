...
prototype/       src/game.ts (оркестрация, реэкспорты), src/prototypeData.ts (инлайн-каталог контента — REFP-1), src/main.ts (UI), src/smoke.ts, build.mjs, uitest.mjs, dist/ (артефакт, в .gitignore)
```
