...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5808 зелёных** (62 skip, 434 файла; с `DATABASE_URL` — 5870 без пропусков: все пропуски — тесты durable-пути, которым нужна база).
