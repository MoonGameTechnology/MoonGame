...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 4605 зелёных** (58 skip, 358 файлов).
...
- В `DEV_MODULES` стоит перед `victoryModule` (кооп-исход по волнам — PVE-4, ещё не
  сделан); `MODULE_MANIFEST_VERSION` поднят `'4'` → `'5'`.
...
## 6. Данные (`data/*.json`, версия `0.1.6`)
...
- **modes** (`modes.json`, `GameModeDefSchema`, PVE-0.1): режим = именованный пресет
  ...
  В каталоге пока один пресет — `standard`: те же числа, что и базовые правила
  `victoryModule` (0.6 / 600 / 0.7), и тест пинит пресет к экспортированным
  константам модуля, чтобы две копии одного факта не разъехались. Пресет ещё НИКУДА
  не подключён: `MatchConfig.modeId` и консервация режима — PVE-0.2.
