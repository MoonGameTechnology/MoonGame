...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5816 зелёных** (70 skip, 435 файлов; с `DATABASE_URL` — 5886 без пропусков). Пропуски — тесты durable-пути и учение бэкапов, которым нужна база; в CI база есть, но нет пакета `age`, поэтому там 5885 и пропущен один тест — реальный криптокруг (проводку шифрования CI проверяет заглушкой, см. §10).
