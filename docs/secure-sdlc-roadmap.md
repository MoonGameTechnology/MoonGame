...
### SD-1.2 · Вывод/экранирование — XSS в клиенте `[cli][proto]` ⏳ — M
...
### SD-1.3 · Инъекции — БД `[srv]` ✅ 2026-09-22 — M
...
**Сделано (2026-09-22).** Первая половина («нет склейки») уже держалась правилом Semgrep
`no-sql-string-interpolation`; вторая («тест на инъекцию через JSON-значение») не
существовала, и без неё правило доказывало только ФОРМУ кода. Теперь есть
`packages/server/src/store/sqlInjection.test.ts` против живого Postgres: ник с
`'; DROP TABLE seats; --` остаётся ником и таблицы целы; SQL-текст внутри ЗНАЧЕНИЙ
`GameState` (имя и фракция игрока) возвращается из JSONB байт-в-байт, а нормализованная
колонка рядом с блобом не страдает; `%` и `_` ищутся как символы, а не как шаблон LIKE;
кавычки и слэши в логине доезжают целыми, повторный логин — `E_LOGIN_TAKEN`, а не второй
аккаунт.

**Находка мутационной проверки: правило видело НЕ ВСЮ склейку.** Прежде чем объявлять
тесты сторожами, поведение проверено мутацией — `seatOf` временно переписан на
конкатенацию (`… WHERE nick = '` + nick + `'`). Живые тесты её поймали (4 падения), а вот
**Semgrep — нет**: его единственная регулярка ловила только интерполяцию в шаблон
(`${}`), и конкатенация прошла бы сканер, упав лишь на настоящей базе. Правило дополнено
второй регуляркой (`pattern-either`), в его тестовый файл добавлены два `ruleid`-случая.
Ограничение формы записано в самом правиле честно: запрос, собранный в переменную
заранее, статике такого вида не виден — его держат тесты на живой базе.

**Правило теперь действует и в `pnpm run check`.** Semgrep гоняется в security-пайплайне,
то есть не в обычном гейте. Сторож в том же тест-файле читает регулярки **из того же
YAML** и прогоняет их по `store/postgres.ts` и `metaMarket.ts` — правилу и сторожу
разъехаться нечем, а склейка падает уже на локальном гейте (проверено той же мутацией).

**Проверка на живой БД воспроизводима локально** без Docker: кластер поднимается
`initdb`/`pg_ctl` из `/usr/lib/postgresql/16/bin` под пользователем `postgres`, и весь
durable-контракт (129 тестов, включая 70 обычно пропускаемых) проходит.

**Остаток — вне этого кирпича:** least-privilege роль БД (`SE-3.1`) и RLS (`SE-3.3`).
...
### SD-6.2 · OIDC вместо долгоживущих секретов `[sec]` ⏳ — S
...
### SD-7.2 · Фаззинг валидаторов и парсеров `[core][srv]` ⏳ (частично ✅) — M

**Сделано (playtest-hardening FUZZ-2, 2026-07-21):** «мусорная» часть на уровне редьюсера —
враждебный payload (`fc.anything()`) × каждый wire-достижимый тип действия (весь каталог
`actionPayloadSchemas`) против `applyAction`: никогда не бросает, только стабильный
`E_*`-код (`applyAction.property.test.ts`, гоняется в каждом `pnpm run check`/CI).
**Остаток:** фаззинг `parseGameData`/`parseClientMessage`/конверта действий случайным
байтовым/структурным входом; сток падений в регресс.
**Готово, когда:** фаззер парсеров гоняется в CI (короткий бюджет) и не находит крашей.

### SD-7.3 · Property-based тесты детерминизма `[core]` ✅ — M

**Сделано (playtest-hardening FUZZ-1…4, 2026-07-21):** fast-check ^4.9 + test-only
`testkit/arbitraries.ts` (типы — из каталога `actionPayloadSchemas`, fixture-вселенная с
seed-варьируемым RNG); три сьюта в каждом `pnpm run check`/CI: `applyAction.property.test.ts`
(не бросает на frozen входах; frozen vs thawed → идентичный исход = не-мутация +
«один seed → один результат» разом; hash переживает JSON; `scheduled` отсортирован),
`advanceTo.property.test.ts` (спаны непрерывны; партиция интервала бит-в-бит на дискретном
ядре, coarse ≈ fine на полном стеке; модуль-бомба дедлеттерится без клина),
`delta.property.test.ts` (`applyDelta∘diffState = id` по `hashState` на sim-достижимых и
синтетических парах + JSON-провод + идемпотентная редоставка). Падение печатает seed —
репро детерминирован.
...
### SD-8.2 · DAST против живого сервера `[sec]` ⏳ — M
