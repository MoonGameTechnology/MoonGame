...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5526 зелёных** (58 skip, 420 файлов).
...
packages/action-layer/src/
  ...
  modules/       army, arsenalSync, artillery, autoRally, capital, captureOnArrival, combat, construction, diplomacy, economy, effects, espionage, faction, fleetOps, fleetRepair, forcedMarch, hero, heroEffects, instantRepair, intercept, market, movement, orbital, planetType, pve, scientist, seatClaim, sector, squadron, standingOrders, station, steward, tax, technology, victory, visibility  (36 модулей, + *.test.ts; сколько из них СОБИРАЕТ каждое ядро — §9)
...
localization/    ВЕСЬ текст для игрока: index.ts (LOCALES/DEFAULT_LOCALE/dataKey), ru.ts, en.ts (плоские карты ключ→текст, 1710 ключей), runtime.ts (ОДИН на прототип и клиент: t/tData/lookup/hasKey/setLocale/localizeStaticDom, LOC-5) + runtime.test.ts. Мост старых msgid снят вместе с LOC-2 — в коде только ключи
...
**Ростер НЕПОЛОН, и из-за этого часть дерева мертва (разбор BAL-6, 2026-09-04).** Учёных
три на пять веток дерева — `overseer:command`, `void_admiral:space`, `polymath` (без
ветки, `slotBonus`), — в обоих каталогах одинаково. У `ground`/`squadron`/`missile`
лидера нет, поэтому капстоуны `planetary_bastions`, `ace_programs` и `saturation_barrage`
гейтятся `has_scientist{branch}` по ветке, которой в ростере не существует: их не может
исследовать никто, в сессии любой длины. Замер (`selfplay 300 base`) это и показывает —
15 достижимых узлов взяты по 600/600, а `deadTech` — ровно эти три плюс `ai_stewardship`
(тот достижим `overseer`'ом, но его `dayGate: 15` выходит за 14-дневное окно замера, при
живой сессии в 30–100 дней — `victory.ts`). Кирпичи: **BAL-13** (достроить ростер),
**BAL-12** (боты в self-play совета не получают вовсе — `matchSetup.ts`, `if (!seat.ai)`,
— поэтому слой `has_scientist` прибору невидим).
...
- `combat.tick` — почасовые раунды: атакующая сторона бьёт `attack`, стоящий
  ...
  **Через хук идут ВСЕ каналы урона** (CORE-DMG-1; решение владельца, ОБРАЩЁННОЕ — первая
  ...
  планеты и бомбардировка в `orbital`, огонь с дистанции в `artillery`, корабельное ПВО в
  `squadron`. Каждый канал зовёт хук со своей фазой — `orbital` / `bombard` / `standoff` /
  `pointDefense`, — и ни одна из них не `ground`, поэтому включились ровно бонусы
  ...
  **Честная граница:** ауры и пассивы героя (`hero.ts`, `heroEffects.ts`) до новых каналов
  ...
  `aa.fired`/`artillery.fired`/`pd.fired` несут теперь ПРИЛЕТЕВШЕЕ число, а не досчётное —
...
**CONV-10 перенёс её в ядро**: модуль в `DEV_MODULES`, копии у прототипа нет, оба
хоста ведут себя одинаково, и переезд игрока с прото-хоста на канон больше не отнимает
привычное поведение. При переносе исправлены два расхождения со словарём канона:
...
- **Состав волны:** `startingLoadout.fleet` NPC-фракции × номер волны (волна N — это
  N стартовых составов). Грубо и намеренно: все числа — в контенте (`data/modes.json`
  — сколько волн и с каким шагом; `data/factions.json` — состав), балансировка волн
...
проход мог ещё не проставить. Ранняя зачистка логова матч НЕ заканчивает: она лишь
лишает будущие волны места спавна, счётчик всё равно доходит до конца, тупика нет.
Существующие исходы не переписаны: весь блок пропускается, если `state.pve` нет.
...
## 6. Данные (`data/*.json`, версия `0.1.9`)
...
> `econplaytest.mjs`) читают ДРУГОЙ каталог — `prototype/src/prototypeData.ts` (1245 строк).
...
> лестницей `upgrades` против отдельных `mine_t1`/`mine_t2`), и правку контента приходится
> делать дважды (так было в BAL-8).
> **Дрейф остановлен, но не сведён (CONV-11):** `prototype/src/contentParity.ts` +
> `contentParity.test.ts` (12 тестов) сверяют каталоги пер-полю и требуют точного совпадения
> с базовым списком `prototype/content-parity-baseline.txt` — **274 расхождения**, снятые
> замером. Красное в ОБЕ стороны: новая строка = дрейф вырос (чинят, а не дописывают в
> список), пропавшая = долг закрыт и запись убирают тем же PR (конвенция `.trivyignore`).
> Гранулярность полевая: иначе новое поле у уже разошедшегося юнита спряталось бы за строкой
> про эту сущность. `version` не сравнивается — это версия бандла, она различается по
> определению. Сам базовый список — готовое ТЗ для CONV-12 (свести к одному источнику, с
> развилкой «какой каталог канон»).
> Практическое следствие прежнее: **числа отсюда не влияют на замер баланса**, его меряет
> только прототипный каталог.
...
- **factions:** шесть — четыре лор-дома (`azure` Azure Compact, `crimson` Crimson
  ...
  (production/speed/combat) и полный `startingLoadout`; `uniqueUnits` пуст у всех.
  `swarm` уже в контенте — отдельная NPC-фракция под PvE заводить не нужно.
...
`forcedMarch`, `instantRepair`, хелперы `squadron.ts`, плюс слоты совета учёных врезкой в
...
(`buildings`) нельзя, пока не решено CONV-12: у прототипа для тех же сущностей другие
имена и частью другие id. (Вопрос вёлся двумя кирпичами — AUD-8 сведён в CONV-12
расчисткой бэклога 2026-09-01.)
...
diplomacy, espionage, botDiplomacy, market, capital, standingOrders, squadron, forcedMarch,
...
- **Командный бой (AVA-0, первый шаг к AvA без мета-слоя):** тумблер «⚔ Командный бой» в
  ...
  **Место закрепляется попаданием на карту, а брошенная заявка истекает:**
  ...
  клиента). Подтверждение ставит транспорт там, где `addPeer` вернул true — один вызов на
  оба хоста, поэтому паритет по построению. Окно — сутки РЕАЛЬНОГО времени
...
- **Конструктор «Верфь» (`rail-constructor` → `shipyard.open()`, оверлей `#constructor`):**
  ...
  подлежит; остальным имя берётся из архетипа каталога — тот же текст, что уже показывают
  чипы ростера, так что шапка и чипы разъехаться не могут. **AUD-13 убрал второй случай
...
- **Карта — «колесо» из десяти секторов (`map.ts`, BAL-1 → BAL-9):** 121 провинция — ровно **30
...
**✅ Этап 1 (ядро) — готово целиком:** **36 модулей** на микроядре (шина/хуки/манифест,
...
**Канон собирает ВСЁ ядро; расхождение осталось односторонним (CORE-PARITY, 2026-08-26).**
В каталоге 36 модулей, и `DEV_MODULES` берёт все 36:
...
`station` и `pve`, которых нет у прототипа (соло не гоняет проекцию тумана — §7). Это
не долг, а незакрытый вопрос «какой набор считается игрой».
...
**Чего в слайсе нет — и это не дыра в фазах, а стык:** `modeId` матчу никто не
ЗАДАЁТ. Прото-хост собирает `MatchConfig` из `timeScale`+`victory` и `MatchMeta` без
`modeId`, а мир считает по второму каталогу (`prototypeData.ts`, где `modes` пуст —
см. «КАТАЛОГОВ ДВА» в §6), так что назначить там режим нельзя: `resolveMatchConfig`
честно отклонит неизвестный. Отсюда же пустует читающая половина, которая УЖЕ
готова: `matchKind()` и `MatchMeta.modeId` в `GET /matches`, значок `PVP`/`PVE` и
фильтр «Режим» в браузере матчей (BRW-2/BRW-4) сегодня не имеют что показывать.
Развязка — CONV-12 (судьба второго каталога) и GM-0.3 (выбор режима в лобби);
`teamFormat` из пресета тоже ждёт этот стык — рассадкой правит `TEAMS`, не режим.
...
**Ботов ДВА, и это разные боты (AI-BAL-1.1).** `aiOrders` принимает профиль:
`basic` (дефолт) — тот простой соперник, которого встречает живой игрок в соло и на
прото-хосте; `test` — лабораторный, его просят только `selfplay.mjs` и `econplaytest.mjs`.
Профиль это АРГУМЕНТ ФУНКЦИИ, а не поле `GameState` и не настройка матча, поэтому получить
тест-бота из игры нечем: в лобби нет такого выбора, в `action`-конверте нет такого поля, в
снапшоте нет такой сущности. Границу стережёт `aiProfile.test.ts` — он читает исходники
игровых путей и краснеет, если `'test'` появился в них.
Ограничение прибора важнее его показаний: **self-play меряет не игру, а то, что умеет
тест-бот**. Базовая линия 2026-08-26 (300 матчей) — матч кончается на 4.1 дня из 60, старт
...
(захватов прилётом 33975 → 28875), AI-BAL-4 — артиллерия и эскадрильи (боёв 19575 → 32625,
мёртвого контента 21 → 8 позиций), **AI-BAL-7 — тактический репертуар** (отступление из
...
Вне охвата осталось то, чего ещё нет в самой игре:
свободный вылет эскадрилий не достроен (`squadron.strike` требует `fleet.homeBase`, которое
не выставляет ни один модуль), поэтому носитель боту не нужен — `backlog.md`, блок AI-BAL.
Приказ `fleet.barrage` боту не нужен по другой причине: с AI-BAL-4 артиллерия стреляет сама,
а `barrage` — только фокус поверх авто-выбора, и лучшего критерия, чем «ближайший», у бота нет.
...
перепроверкой (BAL-6, 2026-09-04):** очереди нет вовсе — берутся ВСЕ 15 достижимых узлов
по 600/600 (оба места × 300 матчей), включая самый дорогой `void_armadas`, то есть
достижимое дерево проходится целиком к 14-му дню и пропускать нечего. Ни цена, ни
`researchTimeHours` не ограничивают. Четыре узла, которые не берутся никогда, отбиты не
ценой, а гейтом `has_scientist` — см. раздел про `scientist` выше и BAL-12/BAL-13.
...
**Симметричная карта обнажила баг прибора:** при РАВНОМ расстоянии до целей тест-бот брал первую
...
`automerge.yml` (ставит зелёный PR в merge queue). Рантайм-база пинена по дайджесту
(`Dockerfile:99`, `gcr.io/distroless/nodejs22-debian13:nonroot@sha256:22d2f048…`, бампнут
SEC-25 2026-08-26). **`.trivyignore` держит теперь ОДНО (SEC-28):** находку, у которой
фикс ОПУБЛИКОВАН, но поставить его мы не можем (сейчас там одна группа — восьмёрка
openssl с фиксом `3.5.7-1~deb13u2`, недоступным внутри distroless — их десять: восемь
из SEC-25 плюс `CVE-2026-14456` и `CVE-2026-75803`, у которых фикс появился уже ПОСЛЕ
их триажа, что и вскрыл гейт). Записи «фикса не существует» — 19 штук — удалены: гейт их и так пропускает, а подавление резало их
