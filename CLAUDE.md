...
получает все языки сразу, клиент грузит РОВНО ОДНУ локаль на старте (LOC-6).
...
репозитории: `err.*` (25 симв.), `cmd.*.hint` (50), `tech.node.*.desc` (59, «Войска
связи: единая картина боя. +8% к радиусу радаров.»). Разбор с цифрами и порядок чистки
