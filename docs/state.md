...
записей при бампе дайджеста. Дисциплина не формальная: `trivy image` идёт без
`--severity`, поэтому один нефиксируемый LOW в базовом образе закрывает очередь мержа
всему репозиторию — так было трижды за август (SEC-22, SEC-24, SEC-25). Про последний важно, что он делает
