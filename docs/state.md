...
- **`fleet.split {fleetId, take[], takeLanding?}`** — отделяет выбранные корабли в новый
  ...
  **Отделение ЧИСТО челночного состава даёт крыло (SQ-1.1):** новый флот получает
  `homeBase` (носитель), и только это отпирает свободный полёт `shuttleModule`
  (`shuttle.strike`/`return` мимо графа линий) — без `homeBase` удар отвергается
  `E_NOT_SHUTTLE`, и весь свободный вылет был недостижимым кодом.
  Два условия здесь неочевидны и оба проверены тестами (`shuttle.test.ts`).
  **Крылом становится только ЧИСТЫЙ shuttle-состав, а не «хотя бы один истребитель»:**
  свободный полёт уносит ВЕСЬ флот, поэтому «хотя бы один» позволял увести крейсер мимо
  графа линий, подцепив его к отделяемым челнокам (`shuttleTake` в `state/shuttle.ts`
  определяет крыло ровно как shuttle-стеки). **Второй координаты крылу не выдают:**
  `shuttle.strike` берёт начало полёта как `freePosition ?? позиция location`, а у
  пристыкованного крыла `location` есть по построению (без него split отказал бы
  `E_IN_TRANSIT`). Выставленная при отделении `freePosition` не обновлялась бы обычным
  ходом по лейну, и крыло, уведённое `fleet.move`, для всей челночной логики
  (`fleetWorldPos` предпочитает `freePosition`) навсегда осталось бы у точки вылета —
  с неверным временем полёта и неверной проверкой радиуса ПВО.
