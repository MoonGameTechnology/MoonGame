...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5688 зелёных** (61 skip, 429 файлов).
...
## 6. Данные (`data/*.json`, версия `0.1.12`)
...
> `econplaytest.mjs`) читают ДРУГОЙ каталог — `prototype/src/prototypeData.ts` (1251 строка).
...
> с базовым списком `prototype/content-parity-baseline.txt` — **216 расхождений** (замер
> 2026-08-27 дал 274; CONV-15 закрыл 13, CONV-17 — ещё 51, оба ниже; HC-3 принёс новый
> контент героя, и к нему правило CONV-17 применено сразу). Красное в ОБЕ стороны: новая
> строка = дрейф вырос (чинят, а не дописывают в список), пропавшая = долг закрыт и
> запись убирают тем же PR (конвенция `.trivyignore`).
...
> имена героев и учёных через `t(name)`. Две живые конвенции — чья каноническая, решает
> CONV-12.
