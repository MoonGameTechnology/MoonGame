...
Итого **1809 ключей**. Записи в локалях разложены по доменным секциям и отсортированы
...
- **UI — тактический пульт (DEFCON-вайб):** векторно-каркасный стиль на чёрном.
  ...
  - **Опциональная флагманская консоль на ПК и планшетах:** переключатель в настройках
    ...
    ширине больше 720 px; телефонная раскладка, в том числе ландшафт с coarse pointer
    и высотой меньше 520 px, остаётся прежней. `pcUi()` и связанные с ним правила
    ...
    **Перемещение окон:** `floatingWindows.ts` позволяет тянуть заголовки
    ...
    голографии/переходе на телефон оба исходных хоста возвращаются на свои места.
    ...
    **Космическая местность** вместо парящих бейджей типа получает векторный узор
    ...
    контур и жёсткие угловые соединения сняты. Простой режим и телефонная
    раскладка сохранены.
  ...
  - **Простой HUD и телефоны — моноширинный, неоновые тонкие линии:** верхний бар в
