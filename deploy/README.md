...
| `TEAMS` | `1v1`/`2v2`/`3v3`/`4v4`/`5v5` (2/4/6/8/10 кресел, две стороны) либо `pve` (двое против сильного бота); пусто = FFA на 10 | пусто |
| `ADMIN_LOGINS` | Кто может командовать составом партии — логины через запятую (`alice,bob`). Полномочие на УЧЁТКЕ: пульт на `/admin` показывает состав и снимает игрока с места, кик пишется в лог с именем администратора, смена пароля отзывает доступ | пусто = админских маршрутов и страницы `/admin` НЕТ вовсе |
| `AUTH_RATE_MAX` / `AUTH_RATE_WINDOW_MS` | Потолок попыток на IP для `/auth/*` | `40` за `60000` мс — хватает целому лобби из-под одного NAT (первый вход = 2 запроса). Поднимайте, если за прокси всё лобби приходит одним адресом |
...
moongame update     # обновление (= deploy/update.sh, файл репозитория: механизм
                    #   обновления приезжает на хост тем же обновлением, что и код)
                    #   проверка здоровья после рестарта + откат при провале
...
С `TRUST_PROXY=1` сервер **требует, чтобы соединение дошло до прокси по HTTPS**
(HTTPS-1.1): WS-upgrade с `X-Forwarded-Proto`, отличным от `https` — в том числе без
заголовка вовсе, — отбивается 403, а причина печатается в лог сервера. Обе заготовки
здесь этот заголовок ставят (Caddy — из коробки, `setup-proxy.sh` — явной строкой), так
что настроенный по этой инструкции стек проходит проверку; если после включения
`TRUST_PROXY=1` клиенты перестали подключаться — смотри лог сервера, там будет названа
причина.

⚠️ **Плейн-порт не должен быть достижим снаружи.** Доверие к `X-Forwarded-Proto` держится
ровно на том, что прислать его может только прокси: клиент, дотянувшийся до открытого
`:8788` напрямую, подделает заголовок и обойдёт проверку. Поэтому за прокси upstream
слушает петлю (`SERVER_BIND=127.0.0.1` в TLS-оверлее ниже) или закрыт файрволом. Сервер
предупреждает об этом на старте, если `TRUST_PROXY=1` стоит вместе с непетлевым `HOST`.
