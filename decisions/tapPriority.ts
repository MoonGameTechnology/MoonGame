 ...
 *
 * 5. **Радиус ОДИН на прицел и на коммит (REFM-125).** По узлу целится не только сам
 *    тап: пока приказ вооружён, превью каждый кадр ищет мир под указателем, чтобы
 *    показать маршрут. И искало СВОЕЙ копией — `tapByTouch ? 30 : 24` литералами прямо
 *    в отрисовке, мимо этой таблицы. Разъедься копии — и превью нарисует путь, которого
 *    отпускание не отправит: игрок увидит линию к миру, а армия туда не пойдёт. Хуже
 *    того, промах будет молчаливым — ни ошибки, ни отказа, просто «приказ не сработал».
 *    Комментарий над копией честно требовал «MUST match selectAt's rNode», но держать
 *    это было нечем.
 */
