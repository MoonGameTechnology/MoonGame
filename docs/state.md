...
Итого **1144 ключа**. Записи в локалях разложены по доменным секциям и отсортированы
...
локалью. Оставшиеся **169** вызовов (`main.ts` 159, `sandbox.ts` 10) ещё держит **мост**
`/localization/legacy/en.ts` (старый msgid = русская строка, 404 записи); мост только
