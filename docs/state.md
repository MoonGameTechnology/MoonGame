...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5524 зелёных** (58 skip, 420 файлов).
...
- `combat.tick` — почасовые раунды: атакующая сторона бьёт `attack`, стоящий
  ...
  **Область хука — ТОЛЬКО этот канал, и это решение владельца, а не регрессия**
  (CORE-DMG-1). Боевые бонусы — пассив фракции, урон от технологий, ауры и способности
  героя, модификаторы форта и типа планеты — усиливают раунд ближнего боя и НЕ доходят до
  остальных каналов, которые бьют «сырым» уроном намеренно: ПВО планеты и бомбардировка
  (`orbital`), огонь с дистанции (`artillery`) и корабельное ПВО против эскадрилий
  (`squadron`, точка `pd.fired`). Бомбардировка при этом вообще не зовёт
  `applyDamageToSide`: она эмитит `planet.bombarded`, а урон по зданиям накладывает
  `construction`. Сам УДАР эскадрилий — не исключение из правила, а его часть: он заводит
  обычный бой (`fleet.arrived` → `engageFleets`) и потому идёт через хук как всякий раунд.
  Границу держит `damageHookScope.test.ts` (5 тестов): каждый канал доказывает, что он
  реально выстрелил И что хук при этом не спрашивали, а раунд ближнего боя — обратное.
  Прежний комментарий `combat.ts` обещал «admiral / tactic / **bombardment** extension
  point» и врал — переписан; у всех трёх «сырых» точек вызова теперь стоит пометка, что
  это умысел, а не забывчивость. Подключение нового канала к хуку — изменение баланса,
  а не уборка.
