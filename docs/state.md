...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5103 зелёных** (58 skip, 389 файлов).
...
25.2k, и боевой пассив crimson множится реже. Что осталось: **88%** территории по-прежнему
меняет хозяина прилётом, snowball 68–72%, кредиты декоративны, дерево технологий — очередь по
...
в СЧЁТЕ, а не в бою.
**BAL-5 разобрал этот остаток и отменил само слово «ком»: УСКОРЕНИЯ НЕТ.** Прибор —
...
«ком не гасится», а «партия решена в первой трети, а идёт ещё две»; рычаги (короче сессия /
карта, которая не кончается за 4 дня / источник очков вне территории) — решение владельца,
кирпич **BAL-10**.
