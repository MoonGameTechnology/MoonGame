...
**Написано ≠ собрано: ядер ДВА, и они собирают РАЗНОЕ.** 36 модулей лежат в каталоге, но
ни одно ядро не берёт все:

| Сборка | Модулей | Чего нет |
| --- | --: | --- |
| `DEV_MODULES` (канонический сервер, `scenario.ts`) | **32** | `effects`, `espionage`, `heroEffects`, `steward` |
...
`shared-core` нет вовсе. Третьим был `autoRally`, но CONV-10 перенёс его в ядро, и теперь
авто-сбор построенного грузят ОБА хоста. Практический смысл расхождения:
партия на **прототипном** хосте (а играют сегодня именно на нём, `launch-runbook.md`)
идёт со шпионажем, «Хранителем» и движком событий, а на **каноническом** сервере — без
них. Самое дорогое следствие отсутствия `heroEffects`: на каноне НЕ РАБОТАЮТ `recall`,
ауры (`rally`/`bulwark`) и весь `scan` вместе с психионной лестницей — каст честно
отвечает `E_NO_EFFECT`, потому что провайдеров `hero.effect.*` в графе нет. Содержимое
`data/*.json` при этом полное: контент шипнут, движок под него на сервере — нет. Обратно: канонический гоняет `visibility`, `station` и `pve`, которых нет у
прототипа. Это не баг и не долг сам по себе — это незакрытый вопрос «какой набор
считается игрой», и до ответа на него любой перенос HUD в `packages/client` упрётся
именно в него.
...
`'8'`). У `MODULES` прототипа такого сторожа нет — он не версионируется поматчево.
