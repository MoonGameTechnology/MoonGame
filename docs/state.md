...
> **КАТАЛОГОВ ДВА, и они СВОДЯТСЯ.** Здесь описан ШИПНУТЫЙ бандл (`data/*.json`, 23 файла,
...
> `econplaytest.mjs`) читают ДРУГОЙ каталог — `prototype/src/prototypeData.ts` (1319 строк).
> Ядро контента не знает (принимает `GameData` как данные), поэтому один движок спокойно
> гоняет оба набора — так они и разъехались до 274 расхождений.
> **Дрейф остановлен (CONV-11), а с CONV-12a повёрнут вспять.** Сторож
> `prototype/src/contentParity.ts` + `contentParity.test.ts` (13 тестов) сверяет каталоги
> пер-полю против базового списка `prototype/content-parity-baseline.txt` — **99 расхождений**
...
> **Что означают оставшиеся 99.** Строк `only-in-prototype` среди них НЕТ НИ ОДНОЙ — это
> отдельный сторож-храповик: `data/*.json` теперь НАДМНОЖЕСТВО играбельного контента.
> Раскладка: 23 сущности, которых у прототипа нет вовсе (4 здания, 3 юнита, 2 фракции,
> 7 режимов, 3 ресурса, 2 события, тип планеты, вид сектора); 43 поля, которых не читает
> никто (`faction` у всех 10 общих юнитов — `blue` против `vanguard`; `scoreValue` у
...
> Практическое следствие ИЗМЕНИЛОСЬ: числа бандла больше не «не влияют на замер» — они
> теперь ТЕ ЖЕ, что меряет прототип. Замер по-прежнему гоняет прототипный каталог, но
> расхождения чисел между двумя каталогами не осталось.
> **Живая базовая линия self-play (снята 2026-09-07 на `f48c086~1`, `selfplay 300` × две
> семьи сидов, 0 ошибок).** Сравнивать новые замеры надо С НЕЙ, а не с числами BAL-8: те
> протухли, и критерий приёмки на них был недостижим сам по себе (CONV-18).
>
> | | семья `base` | семья `alt` |
> | --- | --: | --: |
> | боёв всего | 12370 | 11740 |
> | наземных боёв | 7073 | 6855 |
> | snowball | 71.3% | 75.0% |
> | очки лидера / отставшего | 1701 / 1192 | 1693 / 1207 |
> | разрыв | 509 | 487 |
> | разброс очков по стартам | 4.6% | 10.8% |
> | разброс очков по фракциям | 1.3% | 1.3% |
>
> Разброс по стартам у `alt` вдвое шире, чем у `base`, — это свойство семьи сидов, а не
> регресс: обе сняты на одном и том же дереве. Поэтому семью для сравнения меняют вместе
...
- **Герои — полная новая модель в прототипе:** 5 hero-каталогов инлайн в данных
  `prototypeData.ts` (`game.ts` их только реэкспортирует; зеркало `data/*.json`, те же
  id, что и в легаси-пуле меню), ростер меню (4 героя)
