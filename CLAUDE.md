...
- `decisions/` — **корневая папка, НЕ пакет**: чистые решения, общие обоим клиентам
  ...
  относительным путём, настраивать сборку не надо. Сегодня 115 модулей, их импортируют
  37 файлов прототипа и 6 файлов `packages/client` (без тестов). **Заводя клиентское решение, клади его СЮДА,
