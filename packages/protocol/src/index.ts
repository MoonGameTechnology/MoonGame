 ...
 * silently until `wireParity.test.ts` started catching the drift with type-only Mutual
 * assertions. That test proved the copies matched; it could not stop them from existing.
 *
 * Why a package and not a shared file: the client deliberately has NO dependency on
 * `@void/server` — a relative import across package boundaries would drag server code
 * into the browser bundle. A package both sides depend on removes the mirror instead of
 * policing it, and keeps that boundary intact: nothing here imports the server, and the
 * only dependency is `@void/shared-core` (types the client already ships).
 *
 * What lives here: the message shapes both ends must agree on, the constants that go with
 * them (`PING_KINDS`, `CHAT_CHANNELS`, `CHAT_TEXT_MAX`), and the two functions that turn
