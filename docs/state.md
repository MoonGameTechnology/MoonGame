...
- **Челноки** (shuttles-roadmap; переименовано из «эскадрилий» в SHU-0.1 — трейт
  ...
  **УДАР (SHU-1.2):** `shuttle.strike { planetId | fleetId, squadronId, targetFleetId |
  targetPlanetId }` — вылет из порта по прямой к точке, снятой в момент приказа (цель это
  КООРДИНАТА, а не ссылка: ушла — бьют пустоту). Полёт живёт в `state.strikes`, позиция
  ...
  **КАРТА (вторая половина SHU-3.1, 2026-09-10):** летящий вылет виден. Трасса от базы к
  ...
  повторить фильтр `visibleState`), концы ЖИВЫЕ (носитель мог уйти), а точка удара —
  снимок (самонаведения у вылета нет), ход по времени МИРА, нога решает направление, без
  живой базы трассы нет. Линия сплошная, а не пунктирная, как маршрут флота: маршрут —
