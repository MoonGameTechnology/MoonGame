 ...
 * 4. **Дозорный на маяке не уходит** (`beaconSentinels`). Дозорный — флот Роя с признаком
 *    `sentinel` (его ставят данные карты), стоящий на маяке, которого игрок не взял:
 *    драйверы не отдают ему приказов, не сливают его с другими флотами и не штурмуют им
 *    провинцию. Иначе бот считал бы его свободным отрядом и пускал на расширение — и
 *    задача усиливала бы Рой вне себя (прогон главы против сильного Роя это поймал).
 *    Признак, а не «любой флот Роя на маяке»: так главный флот Роя, зашедший на маяк
 *    по пути, сливался там с дозорным и замирал навсегда (прогон MC-01 это поймал).
 *    Маяк в руках игрока — другое дело: пришедший отряд штурмует его как обычно, иначе
 *    удержание было бы нечем сорвать.
 */
...
/** Признак флота-дозорного маяка (`Fleet.traits`). */
export const SENTINEL_TRAIT = 'sentinel';
...
export function beaconSentinels(state: GameState, npc: PlayerId): Set<FleetId> {
  ...
  for (const f of Object.values(state.fleets))
    if (
      f.owner === npc &&
      f.traits.includes(SENTINEL_TRAIT) &&
      f.location != null &&
      !f.movement &&
      beacons.has(f.location)
    )
      out.add(f.id);
  return out;
}
