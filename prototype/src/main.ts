...
import { initUpdaterUi, RECHECK_EVERY_MS } from './updaterUi';
...
// Сама витрина (баннер, кнопки проверки, дроссель тихих проверок) живёт в `updaterUi.ts`
// (REFM-23), разбор релиза и аллоулист адреса — в `updater.ts`. Здесь только её хуки и три
// места, откуда идёт тихая проверка: старт, возврат приложения на передний план (телефонный
// шаблон — запустили офлайн, открыли позже на wifi) и долгая сессия. В браузерной/дев-сборке
// `currentBuild()` = null, и всё это молча спит.
const updaterUi = initUpdaterUi({
  build: currentBuild,
  check: () => checkForUpdateDetailed(),
  el: (id) => document.getElementById(id),
  now: () => Date.now(),
  online: () => navigator.onLine !== false,
  openExternal: (url) => {
    const native = (globalThis as { VoidNative?: { open?: (u: string) => void } }).VoidNative;
    if (!native?.open) return false;
    native.open(url);
    return true;
  },
  after: (ms, fn) => void window.setTimeout(fn, ms),
});
document.addEventListener('visibilitychange', () => {
  if (!document.hidden) updaterUi.maybeCheck();
});
window.setInterval(() => updaterUi.maybeCheck(), RECHECK_EVERY_MS);
updaterUi.maybeCheck(); // проверка на старте (дроссель штампуется, возврат сразу после неё бесплатен)
