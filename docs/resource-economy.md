...
- **Врезка:** новый модуль `hungerModule` в `game.ts` (в `MODULES` после
  `factionModule`) — вклад в хук `combat.damage`. Контракт хука (`combat.ts`, оба вызова
  `h.hook<number>('combat.damage', …)` в обработчике `combat.tick` — якорь по имени, а не
  по номеру строки: номер уже успел соврать однажды):
  ...
  Через хук идут ВСЕ каналы урона (CORE-DMG-1), но условие ниже стоит на `phase ===
  'ground'` — значит голодная армия слабеет именно в наземной фазе, а ПВО, артиллерия,
  бомбардировка и корабельное ПВО (фазы `orbital`/`standoff`/`bombard`/`pointDefense`)
  голодом не задеты.
