...
export interface DamageHookArgs {
  ...
  battleId?: string;
  /**
   * Флот, НАНОСЯЩИЙ этот удар, когда его наносит флот (CORE-DMG-3). `undefined` —
   * стреляет не флот: планетарное ПВО, перехватчики с мира, гарнизон в бою.
   *
   * Зачем поле, а не вывод по фазе. Потребители из семьи героев (`hero.ts`,
   * `heroEffects.ts`) искали сторону через `battleId`, и это молча делало ауры и
   * пассивы ЧАСТЬЮ БЛИЖНЕГО БОЯ: вне боя `battleId` нет, хук возвращал базу, и
   * обещанные игроку «+8% урона флотам рядом» работали в свалке и молчали на обстреле
   * с орбиты, на челноках и на корабельном ПВО. Таблица «фаза → флот ли это» внутри
   * модуля героев была бы вторым живым описанием боевых каналов рядом с самими
   * каналами — разъехалась бы на первом же новом канале. Канал знает, кто стреляет;
   * он и отвечает.
   */
