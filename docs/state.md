...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5831 зелёный** (70 skip, 436 файлов; с `DATABASE_URL` — 5901 без пропусков). Пропуски — тесты durable-пути и учение бэкапов, которым нужна база; в CI база есть, но нет пакета `age`, поэтому там 5900 и пропущен один тест — реальный криптокруг (проводку шифрования CI проверяет заглушкой, см. §10).
