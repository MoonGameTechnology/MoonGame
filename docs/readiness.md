...
| **1 · Ядро** (`shared-core`)                         | ✅ Готово                        | Модули (ниже; счётчик и состав двух сборок — [`state.md`](state.md) §9, единственный дом этих чисел), микроядро + `advanceTo`, детерминизм (sfc32 + golden), fail-secure, **squadron** (свободный полёт эскадрилий/ракет, `squadron.strike`/`return`/`arrived`), **pointDefense** (реактивное ПВО: залп по эскадрильям в радиусе, перезарядка 20 мин)                                                                                                                                                                                                                                                              | Написанное ≠ собранное: `espionage`/`steward`/`effects`/`heroEffects` есть в каталоге, но НЕ в `DEV_MODULES` канонического сервера — см. `state.md` §9 |
