...
- **инлайновый `parseGameData({ ... })` в `prototype/src/prototypeData.ts`** —
  ...
  `econplaytest`, `netserver`, играбельный клиент. (Исторически каталог лежал инлайном
  в `game.ts`; после REFP он вынесен в отдельный файл, а `game.ts` стал фасадом
  реэкспортов и содержимого каталога больше не несёт.)
...
- задача сформулирована как «отбалансировать, проверить прогоном» → правка нужна в
  каталоге `prototypeData.ts`, и работать надо по скиллу **`balance-analysis`** (там же
  описано, как читать вывод харнесов);
...
Поле `name` в `data/*.json` пишется **по-английски** (`"Sensor Array I"`), а перевод
живёт в `/localization/ru.ts` и `/localization/en.ts` под ключом `data.<слаг>` и
достаётся через `tData()` (рантайм — `localization/core.ts`). Слаг строит `dataKey()`
(`localization/index.ts`): `trim` → нижний регистр → всё, кроме `[a-z0-9]`, в дефис.
`tData('Sensor Array I')` → `data.sensor-array-i`. Отдельной таблицы «имя → ключ» нет,
слаг детерминированный. Ключ нужен **в обеих** локалях сразу — паритет держит тест.
Без записи игрок увидит английское имя: это не крэш, но и не то, что должен увидеть
плейтестер.

**Ловушка, из-за которой имя нельзя писать по-русски.** Слаг вырезает всё, кроме
`[a-z0-9]`, поэтому русское `name` схлопнется в пустой ключ `data.` — перевод станет
недостижим, и игрок на ЛЮБОЙ локали увидит исходную строку. Русский текст такого имени
живёт в `data.*`, а не в JSON.

Id (ключ записи) — `snake_case`, как у всех существующих (`mine_t1`, `strike_carrier`).

Подробности локализации (порядок заведения ключа, что ловит каждый тест гейта, статичная
разметка) — скилл **`localization`**; здесь только то, что касается имён контента.
...
(`fort` HP 35→65, `radar` radarRange 180/300/420, закреплённый СПИСОК из шести
фракций — четыре дома + легаси-пара `vanguard`/`swarm` — в `factions.test.ts`).
Красный тест здесь — не помеха, а сигнал: обнови ожидание
