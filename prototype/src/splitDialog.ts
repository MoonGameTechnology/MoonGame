 ...
 * 4. **Дерётся — окно гаснет.** Ядро делит флот вне боя, так что оставленное окно
 *    обещало бы приказ, который вернётся отказом; а в бою состав вдобавок меняется под
 *    пальцем. Полёт окну не мешает: в пути делить можно (замечание владельца 2026-09-25).
...
export function splitDialogLives(i: SplitLifeInput): boolean {
  ...
  return i.fleetExists && !i.inBattle;
}
