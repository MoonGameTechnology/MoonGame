...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 4890 зелёных** (58 skip, 373 файла).
...
- **Конструктор «Верфь» (`rail-constructor` → `shipyard.open()`, оверлей `#constructor`):**
  ...
  уезжает); плюс `loadoutEditor` в `packages/client` и живая проверка в браузере.
