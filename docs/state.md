...
смыслу; появились исходы `elimination` (3–4%) и разброс длины матча.
**Что показал первый разбор годным прибором** (блок **BAL** в `backlog.md`): старт `C5R1`
берёт 77–80% побед и +62…83% очков — у него 10 призовых миров в радиусе 500 против 6 у
...
прибора:** «фракционный перекос +47% в пользу crimson» перевернулся в +9…15% в пользу azure,
а «снежный ком 100%» стал 91–92%. Всё, что измерялось до AI-BAL-5, требует перепроверки. Не сдвинулись ни
одним кирпичом старт `C5R1` 100% и snowball 100%, и AI-BAL-2 показал, почему: `scoreValue`
есть и у зданий, и у провинций, так что любая постройка приближает победу — снежный ком
живёт в СЧЁТЕ, а не в бою.
...
показывала: фора СТАРТА структурная и растущая (`C5R1` 1535 очков против 914, +68%), а
...
`protoKernel.ts` и `netserver.ts`, и границу стережёт `aiProfile.test.ts`. Отдельно: разные семьи сидов дают
ИДЕНТИЧНЫЙ исход, поэтому «300 матчей» — это 4 конфигурации по 75 повторов, а не 300
наблюдений (AI-BAL-5).
