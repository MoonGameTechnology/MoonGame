...
export class Kernel {
  ...
  /**
   * RULES-1 — «можно ли?» ЗАРАНЕЕ. Возвращает код отказа (`E_*`) или `null`, если
   * действие legально прямо сейчас.
   *
   * ПОЧЕМУ ЭТО НЕ ВТОРОЕ ОПИСАНИЕ ПРАВИЛ. Это тот же `applyAction`: прогон идёт по
   * тем же обработчикам, результат выбрасывается, остаётся вердикт. Дублирования нет
   * ПО ПОСТРОЕНИЮ — не по договорённости и не по комментарию «зеркалит точно» (такой
   * комментарий у нас уже протух молча и стоил бесконечных отказов в интерфейсе).
   *
   * Зачем: редьюсер отвечает только постфактум, а интерфейсу нужно погасить кнопку и
   * назвать причину ДО действия, а автоматике (покадровые циклы, ИИ, драйверы) — не
   * издавать заведомо отвергаемый приказ. Раньше каждый такой потребитель переводил
   * правила руками, и переводы разъезжались.
   *
   * Границы, важные для вызывающего:
   *  · это вопрос ПРАВИЛ ИГРЫ. Сессия, `clientSeq`, идемпотентность живут в
   *    `@void/action-layer` и сюда не входят — «можно по правилам» ≠ «принято сервером»;
   *  · ответ относится к переданному состоянию. Клиент под туманом видит проекцию, и
   *    его вердикт — подсказка, а не решение: авторитет остаётся у сервера;
   *  · чистота: состояние не мутируется (внутри `applyAction` работа идёт на клоне),
   *    поэтому спрашивать можно сколько угодно и в любом порядке.
   */
  canApply(state: GameState, action: Action, ctx: Context): string | null {
    const out = this.applyAction(state, action, ctx);
  ...
  private runStep(
    ...
    // Кости шага. Их может НЕ БЫТЬ, и это законное состояние, а не порча: `visibleState`
    // ВЫРЕЗАЕТ поток RNG из проекции под туманом (клиент с ним прокрутил бы будущие бои
    // вперёд сервера), а сетевой клиент этот же кернел зовёт — через `canApply`, чтобы
    // погасить недоступные приказы. Раньше здесь было `new Rng(draft.rng)` до `try`, и
    // отсутствие костей давало не отказ, а TypeError НАРУЖУ — мимо fail-secure (инвариант
    // #4) и мимо всех, кто ждал `{ ok: false }`. У клиента это убивало цикл отрисовки
    // насмерть (карта замирала на первом же выделении флота), у сервера обрушило бы шаг.
    const rng = draft.rng === undefined ? new AbsentRng() : new Rng(draft.rng);
    ...
    // Косточек не было — не выдумываем их и на выходе: состояние остаётся ровно таким же
    // безрандомным, каким пришло (иначе проекция под туманом обзавелась бы потоком,
    // которого сервер ей не давал).
    if (draft.rng !== undefined) draft.rng = rng.getState();
    draft.time = stepTime;
...
/**
 * Кости, которых нет. Шаг на состоянии БЕЗ потока RNG (проекция под туманом — см.
 * `visibleState`) обязан отвечать вердиктом, а не падать: проверка правил, которая
 * до костей не дотягивается, честно даёт свой ответ, а та, что дотянулась, получает
 * ЧЕСТНЫЙ отказ `E_NO_RNG` вместо тихо подсунутого чужого потока. Подделывать поток
 * нельзя: чужие кости — это молча разошедшееся с сервером предсказание боя.
 *
 * Все броски идут через `nextUint32`, поэтому перекрыт ровно он. `nextInt` с пустым
 * диапазоном до броска не доходит и остаётся рабочим — это не бросок, а тождество.
 */
class AbsentRng extends Rng {
  constructor() {
    super({ a: 0, b: 0, c: 0, d: 0 });
  }
  override nextUint32(): number {
    throw new Rejection('E_NO_RNG');
  }
}
