...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5733 зелёных** (61 skip, 434 файла).
...
- **Челноки** (shuttles-roadmap; переименовано из «эскадрилий» в SHU-0.1 — трейт
  ...
  **УДАР (SHU-1.2):** `shuttle.strike { planetId, unit, count, targetFleetId |
  ...
  ответного огня. Коды: `E_NO_PORT`, `E_PORT_DAMAGED` (порт повреждён более чем на 30%),
