...
Итого **1288 ключей**. Записи в локалях разложены по доменным секциям и отсортированы
по ключу внутри каждой. Статичный узел в разметке ПУСТ, ключ стоит в ЗНАЧЕНИИ атрибута
...
локалью. Вызовов `t('русская строка')` в прототипе **не осталось** (0). **Мост**
`/localization/legacy/en.ts` (старый msgid = русская строка) сократился до **228**
записей, но пока НЕ удаляем: ~**80** вызовов `t(переменная)` получают русский msgid из
таблиц-справочников (`STANCE_RU`, `FORM_RU`, `RES_RU`, `SLOT_RU`, `STAT_RU`,
`HERO_*_RU`, `ARSENAL_*_RU`, `CORP_ROLE_LABEL`, `CORP_AUDIT_RU`, `TECH_FX_LABEL`,
`CALLSIGNS`) и из полей имён/описаний в `prototypeData.ts`, `heroes.ts`,
`formations.ts`, `meta.ts` — мост держит их английский, а гейт этого не видит
(аргумент-переменная разбору литералов не виден). Их миграция и снос моста — остаток
кирпича `LOC-2`; добавлять в мост нельзя. Гейт —
