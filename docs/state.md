...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5743 зелёных** (70 skip, 434 файла; с `DATABASE_URL` — 5813 без пропусков). Пропуски — тесты durable-пути и учение бэкапов, которым нужна база; в CI база есть, но нет пакета `age`, поэтому там 5797 и пропущен один тест — реальный криптокруг (проводку шифрования CI проверяет заглушкой, см. §10).
...
- **Челноки** (shuttles-roadmap; переименовано из «эскадрилий» в SHU-0.1 — трейт
  ...
  **Живут В КОСМОПОРТЕ (SHU-1.1):** `planet.hangar` — не флот и не гарнизон; вместимость
  ...
  не поднимает. Ангар не переживает свой порт: обрезка до вместимости на `time.advanced`
  ...
  **УДАР (SHU-1.2):** `shuttle.strike { planetId, unit, count, targetFleetId |
