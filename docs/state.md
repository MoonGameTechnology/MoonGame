...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5780 зелёных** (70 skip, 436 файлов; с `DATABASE_URL` — 5850 без пропусков). Пропуски — тесты durable-пути и учение бэкапов, которым нужна база; в CI база есть, но нет пакета `age`, поэтому там 5849 и пропущен один тест — реальный криптокруг (проводку шифрования CI проверяет заглушкой, см. §10).
...
- **Челноки** (shuttles-roadmap; переименовано из «эскадрилий» в SHU-0.1 — трейт
  ...
  **НОСИТЕЛЬ — ВТОРАЯ БАЗА (SHU-2.1):** `shuttleBay` стал ещё и статом КОРПУСА, поэтому
  ...
  навсегда. Отказ игроку — `E_TECH_LOCKED` (`err.tech-locked`, заведён в обе локали). **Имя игроку — «Орбитальное ПКО»**
