...
## Телефонный тактический цикл — 12 сентября

`prototype/mobiletest.mjs` проверяет реальный браузер с touch-вводом: 390×844,
320×740, 844×390, переход на планшетную раскладку и обратно. Тест использует
локальный сервер, закреплённый Playwright и мост только для чтения состояния;
изменения игры выполняются кнопками и касаниями. Dev/player HTML запускаются также
без моста. Отдельный контекст мыши проверяет прежнюю отправку «Курса» на ПК.

Проверено: единственная компактная карточка, раскрытие сведений и характеристик,
Back, выбор провинции из списка под флотом, панорама и щипок при выборе цели,
сохранение адреса цели, отмена без изменения игры, реальная отправка и «Стоп»,
выход из прицеливания после «Ещё», пустая группа с доступной кнопкой «Готово»,
композер пинга для неизвестной провинции. Поворот сохраняет объект в видимой части
карты, повторная смена раскладки не дублирует `#side`/`#cmdbar`. Горизонтального
переполнения и ошибок страницы нет. Проверены общий гейт и UI smoke.

Холодная векторная местность использует существующий рендер и настройки.
Общий скруглённый контур перенесён из #975 для того же рендера на телефоне;
ни геометрия игровых областей, ни проверки приказов не менялись.
Проверка эмулирует телефонный ввод в Chromium; она не измеряет FPS и расход
батареи на физическом устройстве. Стратегические полноэкранные меню — следующий этап.
Новые версии Sites не создавались.
...
Ошибок страницы нет. На 834×1112 окно остаётся доступным. Телефонная компоновка
после следующего этапа проверяется отдельно ниже.
