...
SEC-34 2026-09-08). Бамп закрыл ДЕСЯТЬ CVE, а не ноль, как записал сам SEC-34: в новой базе
`libssl3t64 3.5.7-1~deb13u2` против `3.5.6-1~deb13u2` в заменённой — прочитано из
`/var/lib/dpkg/status.d` обоих образов в SEC-38. Прогон Trivy этого показать не мог, потому
что группа была подавлена, а подавление режет находку и из ОТЧЁТА; отсюда правило: результат
бампа сверяется версиями пакетов из образа, а не диффом отчёта. **`.trivyignore` держит
теперь ОДНО (SEC-28):** находку, у которой фикс ОПУБЛИКОВАН, но поставить его мы не можем.
Openssl-десятка снята в SEC-38 (фикс приехал); сейчас там одна группа — `CVE-2026-5450`
(`scanf` с `%mc`) и `CVE-2026-5928` (`ungetwc`) в `libc6 2.41-12+deb13u3` с фиксом
`2.41-12+deb13u4`, которого нет ни в одной линии distroless (проверено по gcr.io и
upstream-lockfile), ре-ревью 2026-09-19. Обе разобраны по самому образу: `ungetwc` нет среди
импортов бинаря `node`, `%mc` не встречается в нём ни разу, других исполняемых файлов в
образе нет. Записи «фикса не существует» — 19 штук — удалены: гейт их и так пропускает, а подавление резало их
