...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5832 зелёных** (62 skip, 436 файлов; с `DATABASE_URL` — 5894 без пропусков: все пропуски — тесты durable-пути, которым нужна база).
...
- **Очередь стройки (BLD-1, `Planet.buildQueue`).** Мир строит по ОДНОЙ вещи в полосе;
  ...
  намерение, ровно то, за что `visibleState` режет `scheduled` и цепочки приказов.
