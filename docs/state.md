...
> **Гейт:** `pnpm run check` (lint + typecheck + test + docs-check). **Тесты: 5770 зелёных** (62 skip, 433 файла; с `DATABASE_URL` — 5832 без пропусков: все пропуски — тесты durable-пути, которым нужна база).
...
- **Челноки** (shuttles-roadmap; переименовано из «эскадрилий» в SHU-0.1 — трейт
  ...
  **УДАР (SHU-1.2):** `shuttle.strike { planetId, unit, count, targetFleetId |
  ...
  выводится из времени. Урон ОДНОСТОРОННИЙ: флоту через хук `combat.damage`
  (`phase: 'shuttle'`), миру событием `planet.bombarded` (урон зданиям) — ни боя, ни
  ...
  **НОСИТЕЛЬ — ВТОРАЯ БАЗА (SHU-2.1):** `shuttleBay` стал ещё и статом КОРПУСА, поэтому
  ...
  ПВО», `data.area-defense-array`), а одноимённое поле у ЗДАНИЙ пока не читает никто
  (оживёт в ROS-2.2 вместе со зданием). Внутренние id обоих не менялись: игроку они не
