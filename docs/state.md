...
- **Слоты под скиллы: владеть ≠ носить (HPR-1.2, модуль `hero` v1.1.0).** `Hero.abilities`
  — ПУЛ (чем владеет: стартовые способности архетипа + гранты дерева и фиттингов),
  ...
  ступень — базовый дефолт в один слот. **Это НЕ тот же бюджет, что `slots` архетипа:** тот
  ограничивает `hero.fit` (компоненты корабля), этот — `hero.equip` (способности); числа
  обманчиво похожи и лестницы обратные. Действия **`hero.equip`/`hero.unequip`
  {heroId, abilityId}**: владение (`E_NOT_OWNED`) → generic-гейт `canInstall`
  (`util/fitting.ts`, тот же, что у модулей и фиттингов) → `E_NO_ABILITY`/
  `E_ALREADY_EQUIPPED`/`E_NO_SLOTS`; события `hero.equipped`/`hero.unequipped`.
  Обратимо намеренно, в отличие от `hero.fit`: фиттинг приваривается навсегда, а слот
  скилла снимается — расклад, который нельзя переставить, это ловушка. Стоимости нет.
...
- **Герои — полная новая модель в прототипе:** 5 hero-каталогов инлайн в данных
  ...
  **все не-встроенные эффекты имеют провайдеры → «скоро» не осталось**), дерево
  `hero.skill.unlock`, фиттинги `hero.fit`, **слоты под скиллы `hero.equip`/`hero.unequip`
  ...
  фолбэк и как единственный путь с клавиатуры). Вкладка «Фиттинги» переведена на ту же
  идиому отсеков с ОДНИМ намеренным отличием: занятый отсек не предлагает «снять», потому
  что `hero.fit` необратим, — асимметрия видна ровно потому, что всё остальное совпадает**. Кастуемость — `HERO_CASTABLE` (built-ins +
