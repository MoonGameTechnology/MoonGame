...
PVE-0.2 modeId в MatchConfig       ✅ резолв в конструкторе MatchRoom, E_UNKNOWN_MODE
   │
   ├─ PVE-1.* Командные форматы    ← СЛЕДУЮЩИЕ: teamFormat, networkSeats, пресеты
   │     PVE-1.1 TeamFormat тип
   │     PVE-1.2 пресеты 1v1..5v5
   │     PVE-1.3 локализация (в одном PR с 1.2)
   │
   ├─ PVE-3.* pveModule            ← готов к работе (PVE-0.2 закрыт)
   │     PVE-3.1 state.pve
   │     PVE-3.2 спавн волн
...
**Порядок сборки (остаток):** PVE-1.* (командные форматы) → PVE-3.* (`pveModule`)
→ PVE-4.* (победа) → PVE-5.* (AI, опц.) → PVE-6.* (доки). Фаза 0 закрыта целиком.
