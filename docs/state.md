...
Итого **1557 ключей** — на обе поверхности сразу: рантайм (`/localization/runtime.ts`,
выбор языка + `t()`/`tData()`) ОБЩИЙ для прототипа и PWA-клиента, `prototype/src/i18n.ts`
поверх него — тонкая обёртка со своим `localizeStaticDom()`. Клиент подключён к той же
папке кирпичом LOC-3: `cli.*` (экран приветствия, статусы сети, отказы), `loadout.stat.*`
(подписи характеристик) и общие с прототипом ключи (`welcome.*`, `callsign.*`). Записи в локалях разложены по доменным секциям и отсортированы
...
ключи, промах виден как сам ключ. Гейт — `prototype/src/i18n.test.ts` (9 тестов, читает И `prototype/src`, И `packages/client/src`:
...
отдают ключи, в файлах локалей нет дублирующихся ключей, и — главный инвариант
этапа 2 — в коде нет русского текста). Тест сирот
