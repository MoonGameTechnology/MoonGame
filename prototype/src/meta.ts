/**
 * META-PROGRESSION (прокачка командующего) — СВОЯ ось, не ось аккаунта (PVR-3.1).
 *
 * ⚠️ Здесь эти деревья назывались деревьями УРОВНЯ АККАУНТА, и из той формулировки
 * выросло противоречие: `docs/account-level.md` держит правило «уровень аккаунта НИКОГДА не
 * сила», а это дерево силу именно даёт — скрытые техи, уровень совета, множитель
 * казны. Пока оно называло себя осью аккаунта, два документа описывали один термин
 * несовместимыми правилами. Решение — `account-level.md` §1.1: осей три, и эта —
 * ось КОМАНДУЮЩЕГО, со своим домом (`vd.meta.<позывной>`) и своим опытом.
 *
 * Прежнее обещание «серверный аккаунт подхватит это дерево, когда мета туда приедет»
 * ОТОЗВАНО тем же решением: переезд слил бы силовую ось в power-free, то есть сломал
 * бы правило аккаунта молча. Серверный `CommanderStore` остаётся счётчиком оси
 * аккаунта; захочет владелец перенести дерево на сервер — это отдельное решение с
 * собственным домом, а не «мета приехала».
 *
