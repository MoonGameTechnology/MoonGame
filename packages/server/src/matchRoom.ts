...
export class MatchRoom {
  ...
  /**
   * RULES-3. «Приняли бы всю эту ПОСЛЕДОВАТЕЛЬНОСТЬ приказов?» — вердикт ядра без
   * применения к комнате: первый код отказа или `null`, если проходит целиком.
   *
   * Нужен серверным драйверам постоянных приказов. Они издают приказы от лица игрока и
   * раньше переписывали правила ядра своими условиями, чтобы не сыпать обречёнными
   * приказами каждый тик. Вопрос именно про последовательность, а не про один приказ:
   * авто-штурм издаёт ПАРУ «встать на низкую орбиту → штурм», и штурм нелегален с
   * дальней орбиты — спросив только про него, драйвер получил бы `E_WRONG_ORBIT` про
   * мир ДО первого действия пары и не выдал бы приказ никогда. Спросив только про
   * орбиту — применил бы половину обречённой пары.
   *
   * Читающий вход: работает на ЧЕРНОВИКЕ (`applyAction` возвращает новое состояние,
   * инвариант #2), `this.stateValue` не трогается.
   */
  canApplyAll(state: GameState, actions: readonly Action[], now: number): string | null {
    return this.kernel.canApplyAll(state, actions, this.context(now));
  }
