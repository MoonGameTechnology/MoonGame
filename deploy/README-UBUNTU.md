...
bash /opt/moongame/deploy/update.sh
```
...
Сам скрипт обновления — **файл репозитория** (`deploy/update.sh`), поэтому его правки
приезжают на хост обычным обновлением. Раньше он генерировался установщиком в
`/opt/moongame/update-dev.sh` и на диск попадал ровно один раз: любая починка механизма
обновления до уже развёрнутой машины не доезжала.

### Разовая миграция машины, установленной до этого изменения

На таком хосте хелпер всё ещё зовёт старую копию. Один раз, руками:

```bash
cd /opt/moongame && moongame update          # старой копией — притащить свежий код
sudo sed -i 's#\$INSTALL_DIR/update-dev.sh#$INSTALL_DIR/deploy/update.sh#' /usr/local/bin/moongame
rm -f /opt/moongame/update-dev.sh            # копия больше не нужна
```

Дальше `moongame update` исполняет уже репозиторный скрипт, и следующие правки механизма
приедут сами.
