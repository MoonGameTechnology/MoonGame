...
**Чего нет:** 1v1/3v3/4v4 в `NetworkMatchMode` (Фаза 1), кооп-условия победы по
волнам (Фаза 4), AI-оркестратора PvE (Фаза 5). Фазы 0, 2 и 3 закрыты.
...
PVE-0.2 modeId в MatchConfig       ✅ резолв в конструкторе MatchRoom, E_UNKNOWN_MODE
   ...
   │
   ├─ PVE-4.* Кооп-условие победы  ← СЛЕДУЮЩИЕ (PVE-3.2 закрыт)
...
**Порядок сборки (остаток):** PVE-4.* (кооп-победа + пресет `pve_waves`) → PVE-1.*
(командные форматы) → PVE-5.* (AI, опц.) → PVE-6.* (доки). Фазы 0, 2 и 3 закрыты.
